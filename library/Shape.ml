...
type member = {
  name: string ;
  target: string ;
  traits: Trait.t list option }[@@deriving (show, equal)]
type structureShapeDetails =
  {
  traits: Trait.t list option ;
  members: member list }[@@deriving (show, equal)]
type setShapeDetails = {
  traits: Trait.t list option ;
  target: string }[@@deriving (show, equal)]
...
type shapeDescriptor =
  ...
  | SetShape of setShapeDetails [@@deriving (show, equal)]
let getShapeTraits descriptor =
  match descriptor with
  | ((ListShape ({ traits;_}))[@explicit_arity ]) | ((StructureShape
    ({ traits;_}))[@explicit_arity ]) | ((OperationShape
    ({ traits;_}))[@explicit_arity ]) | ((UnionShape
    ({ traits;_}))[@explicit_arity ]) | ((BlobShape
    ({ traits }))[@explicit_arity ]) | ((ServiceShape
    ({ traits;_}))[@explicit_arity ]) | ((BooleanShape
    ({ traits }))[@explicit_arity ]) | ((IntegerShape
    ({ traits }))[@explicit_arity ]) | ((StringShape
    ({ traits }))[@explicit_arity ]) | ((MapShape
    ({ traits;_}))[@explicit_arity ]) | ((TimestampShape
    ({ traits }))[@explicit_arity ]) | ((LongShape
    ({ traits }))[@explicit_arity ]) | ((FloatShape
    ({ traits }))[@explicit_arity ]) | ((DoubleShape
    ({ traits }))[@explicit_arity ]) | ((BigIntegerShape
    ({ traits }))[@explicit_arity ]) | ((BigDecimalShape
    ({ traits }))[@explicit_arity ]) | ((SetShape
    ({ traits;_}))[@explicit_arity ]) -> traits
  | ResourceShape -> None
type t = {
  name: string ;
  descriptor: shapeDescriptor }[@@deriving (show, equal)]
let isServiceShape shape =
  match shape with | ServiceShape _ -> true | _ -> false