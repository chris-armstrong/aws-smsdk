...
let partitionOperationShapes shapesWithTargets =
  let (service, remaining) =
    List.partition_map shapesWithTargets
      ~f:(fun { descriptor; name; targets; recursWith } ->
            match descriptor with
            | ((ServiceShape (x))[@explicit_arity ]) ->
                ((Base.Either.First ((name, x))))
            | _ ->
                ((Base.Either.Second
                    ({ name; descriptor; targets; recursWith }))
                [@explicit_arity ])) in
  let (operations, structured) =
    List.partition_map shapesWithTargets
      ~f:(fun { descriptor; name; targets; recursWith } ->
            match descriptor with
            | ((OperationShape (x))[@explicit_arity ]) ->
                ((Base.Either.First ((name, x, targets))))
            | _ ->
                ((Base.Either.Second
                    ({ name; descriptor; targets; recursWith }))
                [@explicit_arity ])) in
...
let operationDependencies (operationDetailsList : operationTriple list) =
  (operationDetailsList |> (List.map ~f:(fun (_, _, targets) -> targets))) |>
    List.concat
    ...
    (List.partition_tf
       ~f:(function
           | Shape.{ name; descriptor = StructureShape _ }  ->
               List.exists allDependencies
                 ~f:(fun dependency -> String.equal name dependency)
           | _ -> false))
