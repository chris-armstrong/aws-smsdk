...
let rec generateStructureSerialiserFuncBody fmt name (descriptor : structureShapeDetails) =
  Fmt.pf fmt "(x: %s) => assoc_to_yojson(@;<0 2>[@[<v 2>" (SafeNames.safeTypeName name);
...
and generateSerialiserFuncBody fmt name shapeDescriptor =
  ...
  | ((MapShape x) [@explicit_arity]) ->
      Fmt.pf fmt "(x) => map_to_yojson(%s, x)" (generateSerialiserFuncName x.mapValue.target)
  | ((UnionShape x) [@explicit_arity]) -> generateUnionSerialiserFuncBody fmt name x
  | ((SetShape x) [@explicit_arity]) ->
      Fmt.pf fmt "(x) => list_to_yojson(@;<0 2>%s@;,x@;)" (generateSerialiserFuncName x.target)
...
let descriptorHasBody = function
  | ((StructureShape x) [@explicit_arity]) -> not (Trait.hasTrait x.traits Trait.isErrorTrait)
  ...
  | ListShape _ | FloatShape _ | DoubleShape _ ->
      true
...
let generateOperations fmt serviceName operationShapes =
  ...
  |> List.iter ~f:(fun (name, os) ->
         Fmt.pf fmt "module %s {@;<0 2>@[<v>" (SafeNames.safeConstructorName name);
         let requestShape =
           os.input
