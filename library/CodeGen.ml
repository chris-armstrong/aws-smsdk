...
let escapeString str =
  (str |> (fun __x -> Str.global_replace reBSlashBSlash "\\\\" __x)) |>
    (fun __x -> Str.global_replace reBSlashDQuote "\\\"" __x)
  ...
  ((let docStrs =
      (traits |> (Option.value ~default:[])) |>
        (List.filter_map
           ~f:(fun trait ->
                 match trait with
                 | ((Trait.DocumentationTrait (str))[@explicit_arity ]) ->
                     ((Some (str))[@explicit_arity ])
                 | _ -> None)) in
    let docs = String.concat docStrs ~sep:"" in
    if not (String.is_empty docs)
    then ("@ocaml.doc(\"" ^ (escapeString docs)) ^ "\")"
    else "")
  [@ns.braces ])
...
let generateStringShape (details : Shape.primitiveShapeDetails) =
  ((let enumTrait =
      let open Option in details.traits >>= (List.find ~f:Trait.isEnumTrait) in
...
let generateMember (m : Shape.member) ?(genDoc= false)  () =
  ((let safeName = safeMemberName m.name in
    ...
    let valueType =
      ((if required
        then safeTypeName m.target
        else ("option(" ^ (safeTypeName m.target)) ^ ")")
      [@ns.ternary ]) in
    let doc =
      match genDoc with
      | true -> ((Some ((generateDoc m.traits)))[@explicit_arity ])
      | false -> None in
    generateField safeName valueType ?doc)
  [@ns.braces ])
let indentString indent =
  ((let is = [||] in
    Array.fill is ~len:((indent)[@ns.namedArgLoc ]) ~pos:((0)
      [@ns.namedArgLoc ]) " ";
    String.concat_array is)
  [@ns.braces ])
let generateStructureShape (details : Shape.structureShapeDetails) ?(genDoc=
  false)  () =
  ((generateRecordTypeDefinition
      (List.map details.members
         ~f:(fun member -> generateMember member ~genDoc ())))
  [@ns.braces ])
let generateUnionShape (details : Shape.structureShapeDetails) ?(genDoc=
  false)  () =
  let tConstructors =
    List.map details.members
      ~f:(fun member ->
            (((safeVariantName member.name) ^ "(") ^
               (safeTypeName member.target))
              ^ ")") in
  let t = String.concat tConstructors ~sep:" | " in t
...
let generateTimestampShape ({ traits } : Shape.timestampShapeDetails) =
  ((let timestampFormat =
      Trait.findTrait (Option.value traits ~default:[])
        Trait.isTimestampFormatTrait in
    match timestampFormat with
    | ((Some
        (((TimestampFormatTrait ("date-time"))[@explicit_arity ])))[@explicit_arity
                                                                    ])
        -> {js|float|js}
    | ((Some
        (((TimestampFormatTrait ("epoch-seconds"))[@explicit_arity ])))
        [@explicit_arity ]) -> {js|float;|js}
    | _ -> {js|float|js})
  [@ns.braces ])
...
let generateMake input =
  ...
  | ((OperationStructure ({ members;_}))[@explicit_arity ]) ->
      ((let (requiredMembers, optionalMembers) =
          List.partition_tf members
            ~f:(fun x -> Trait.hasTrait x.traits Trait.isRequiredTrait) in
        let requiredArguments =
          List.map requiredMembers
            ~f:(fun member -> {js|~|js} ^ (safeMemberName member.name)) in
        let optionalArguments =
          List.map optionalMembers
            ~f:(fun member ->
                  ({js|~|js} ^ (safeMemberName member.name)) ^ {js|=?|js}) in
        let arguments =
...
let generateTypeTarget descriptor ?(genDoc= false)  () =
  ((let open Shape in
      ...
      | ((StringShape (details))[@explicit_arity ]) ->
          generateStringShape details
      | ((StructureShape (details))[@explicit_arity ]) ->
          generateStructureShape details ~genDoc ()
      | ((ListShape ({ target;_}))[@explicit_arity ]) ->
          generateListShape target
      ...
      | ((MapShape (details))[@explicit_arity ]) ->
          generateMapShape details.mapKey details.mapValue
      | ServiceShape _ -> ""
      | ((UnionShape (details))[@explicit_arity ]) ->
          generateUnionShape details ~genDoc ()
      | ((TimestampShape (details))[@explicit_arity ]) ->
          generateTimestampShape details
      | ResourceShape -> ""
      | OperationShape _ -> ""
      | ((SetShape (details))[@explicit_arity ]) -> generateSetShape details)
  [@ns.braces ])
let getStructureShape =
  let open Shape in
    function
    | ((StructureShape (details))[@explicit_arity ]) -> ((Some (details))
        [@explicit_arity ])
    | _ -> None
let generateExceptionBlock name _ =
  "exception " ^
    ((safeConstructorName name) ^ "(Aws.apiError(Aws.emptyErrorDetails))")
let generateTypeBlock ({ name; descriptor } : Shape.t) ?(genDoc= false)  () =
  let docs =
    match genDoc with
    | true -> generateDoc (Shape.getShapeTraits descriptor)
    | false -> "" in
  let result = generateTypeTarget descriptor ~genDoc:false () in
  let t = ((if String.equal result "" then "" else generateType name result)
    [@ns.ternary ]) in
  match descriptor with
  | ((StructureShape (details))[@explicit_arity ]) when
      Trait.hasTrait details.traits Trait.isErrorTrait ->
    ...
    List.map shapes
      ~f:(fun shape ->
            ((safeTypeName shape.name) ^ " = ") ^
              (generateTypeTarget shape.descriptor ~genDoc ())) in
  let shapeModules =
    (shapes |>
       (List.filter_map
          ~f:(fun Shape.{ name; descriptor }  ->
                match descriptor with | _ -> None)))
      |> (String.concat ~sep:"\n") in
