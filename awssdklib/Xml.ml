...
module Parse =
  struct
    let source_with_encoding ~src  ~encoding  =
      let enc =
        Option.bind encoding
          ~f:(fun encoding ->
                match String.lowercase encoding with
                | "utf-8" -> ((Some (`UTF_8))[@explicit_arity ])
                | _ -> None) in
    ...
    type expected =
      ...
      | XmlDtd 
      | XmlOneOfElement of string list [@@deriving (show, eq)]
    type pos = (int * int)
    ...
    module Accept =
      struct
        let startTag i tag ~ns  ~expected  =
          let next = input i in
          match next with
          | `El_start ((nns, nname), attributes) when
              ((Option.is_none ns) ||
                 (String.equal nns (Option.value ns ~default:"")))
                && (String.equal nname tag)
              -> (nns, nname, attributes)
          | _ ->
              raise ((XmlUnexpectedConstruct (expected, next, (Xmlm.pos i)))
                [@explicit_arity ])
        ...
        let dtd i =
          ...
          | `Dtd _ -> ()
          | _ as va ->
              raise ((XmlUnexpectedConstruct (XmlDtd, va, (Xmlm.pos i)))
                [@explicit_arity ])
      end
    ...
    module Read =
      struct
        ...
        let sequence i tag (reader : 'a reader) ?ns  () =
          ...
          let _ =
            Accept.endTag i ~expected:((XmlEndSequence (tag, ns))
              [@explicit_arity ]) in
          res
        let element i tag ?ns  () =
          let (_, _, _) =
            Accept.startTag i tag ~ns ~expected:((XmlStartElement (tag, ns))
              [@explicit_arity ]) in
          let data =
            Accept.data i ~expected:((XmlElementData (tag, ns))
              [@explicit_arity ]) in
          let _ =
            Accept.endTag i ~expected:((XmlEndElement (tag, ns))
              [@explicit_arity ]) in
          data
        ...
        let optionalElements i tag ?ns  () =
          ...
          match List.is_empty elements with
          | true -> None
          | false -> ((Some (elements))[@explicit_arity ])
        let optionalElement i tag ?ns  () =
          match Xmlm.peek i with
          | `El_start el when tag_equal tag ns el ->
              ((Some ((element i tag ?ns ())))[@explicit_arity ])
          | _ -> None
    ...
    module Structure =
      struct
        type 'returnType inputType =
          ...
          | InputStructureElement: 'returnType Read.reader -> 'returnType
          inputType 
          | InputStructuresElement: 'returnType Read.reader -> 'returnType
          list inputType 
        type 'a inputItem = {
          tag: string ;
          type_: 'a inputType }
            ...
            | `Dtd _ | `Data _ ->
                raise
                  ((XmlUnexpectedConstruct
                      (((XmlOneOfElement (expectedTags))[@explicit_arity ]),
                        next, (Xmlm.pos i)))[@explicit_arity ])
            done
        let item2 : type a b.
          input -> a inputItem -> b inputItem -> (a option * b option) =
          fun i ->
            fun t1 ->
              fun t2 ->
                let r1 = ref None in
                let r2 = ref None in
                scanSequence i [t1.tag; t2.tag]
                  (fun tag ->
                     fun next ->
                       if String.equal t1.tag tag
                       then r1 := (item1 i t1)
                       else
                         if String.equal t2.tag tag
                         then r2 := (item1 i t2)
                         else
                           raise
                             ((XmlUnexpectedConstruct
                                 (((XmlOneOfElement ([t1.tag; t2.tag]))
                                   [@explicit_arity ]), next, (Xmlm.pos i)))
                             [@explicit_arity ]));
                ((!r1), (!r2))
        let item3 : type a b c.
          input ->
            a inputItem ->
              b inputItem -> c inputItem -> (a option * b option * c option)
          =
          fun i ->
            fun t1 ->
              fun t2 ->
                fun t3 ->
                  ...
                  let r2 = ref None in
                  let r3 = ref None in
                  scanSequence i [t1.tag; t2.tag; t3.tag]
                    (fun tag ->
                       fun next ->
                         match tag with
                         | _ when String.equal t1.tag tag ->
                             r1 := (item1 i t1)
                         | _ when String.equal t2.tag tag ->
                             r2 := (item1 i t2)
                         | _ when String.equal t3.tag tag ->
                             r3 := (item1 i t3)
                         | _ ->
                             raise
                               ((XmlUnexpectedConstruct
        ...
        let item4 : type a b c d.
          input ->
            a inputItem ->
              b inputItem ->
                c inputItem ->
                  d inputItem -> (a option * b option * c option * d option)
          =
          fun i ->
            fun t1 ->
              fun t2 ->
                fun t3 ->
                  fun t4 ->
                    ...
                    let r3 = ref None in
                    let r4 = ref None in
                    scanSequence i [t1.tag; t2.tag; t3.tag; t4.tag]
                      (fun tag ->
                         fun next ->
                           match tag with
                           | _ when String.equal t1.tag tag ->
                           ...
                           | _ when String.equal t3.tag tag ->
                               r3 := (item1 i t3)
                           | _ when String.equal t4.tag tag ->
                               r4 := (item1 i t4)
                           | _ ->
                               raise
                                 ((XmlUnexpectedConstruct
                ...
                fun t3 ->
                  fun t4 ->
                    fun t5 ->
                      ...
                      let r4 = ref None in
                      let r5 = ref None in
                      scanSequence i [t1.tag; t2.tag; t3.tag; t4.tag; t5.tag]
                        (fun tag ->
                           fun next ->
                             match tag with
                             | _ when String.equal t1.tag tag ->
                             ...
                             | _ when String.equal t4.tag tag ->
                                 r4 := (item1 i t4)
                             | _ when String.equal t5.tag tag ->
                                 r5 := (item1 i t5)
                             | _ ->
                                 raise
                                   ((XmlUnexpectedConstruct
                  ...
                  fun t4 ->
                    fun t5 ->
                      fun t6 ->
                        ...
                        let r6 = ref None in
                        scanSequence i
                          [t1.tag; t2.tag; t3.tag; t4.tag; t5.tag; t6.tag]
                          (fun tag ->
                             fun next ->
                               match tag with
                               | _ when String.equal t1.tag tag ->
                               ...
                               | _ when String.equal t5.tag tag ->
                                   r5 := (item1 i t5)
                               | _ when String.equal t6.tag tag ->
                                   r6 := (item1 i t6)
                               | _ ->
                                   raise
                                     ((XmlUnexpectedConstruct
                          ...
                          scanSequence i
                            ...
                            (fun tag ->
                               fun next ->
                                 ...
                                 | _ when String.equal t6.tag tag ->
                                     r6 := (item1 i t6)
                                 | _ when String.equal t7.tag tag ->
                                     r7 := (item1 i t7)
                                 | _ ->
                                     raise
                                       ((XmlUnexpectedConstruct
    ...
    let required tag value i =
      ...
      | None ->
          raise ((XmlMissingElement (tag, (Xmlm.pos i)))[@explicit_arity ])
  end