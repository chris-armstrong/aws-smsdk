...
module Request = struct
  ...
  let make ~(service : Aws.Service.descriptor) ~context ~action ~(fields : field option list) ~sw
    ...
    let auth = context.config.resolveAuth () in
    let region = context.config.resolveRegion () in
    let headers =
  ...
  let parse_xml_error_response ~(body : string) =
    ...
    try
      Read.sequence xmlSource "ErrorResponse"
            ...
            Read.sequence xmlSource "Error"
              (fun i _ ->
                ...
                let errorType =
                  ...
                  | _ ->
                      raise
                        (Unparseable ("Unknown Error type (expected Sender/Receiver)", body)
                        [@explicit_arity])
                in
                let open Error in
                { errorType; code })
        ...
        ()
    with
    | Xml.Parse.XmlParse _ -> raise (Unparseable ("xmlm error", body) [@explicit_arity])
