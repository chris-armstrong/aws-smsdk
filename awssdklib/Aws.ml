  ...
  struct
    ...
    let amzDateTime now =
      let Unix.{ tm_year; tm_mon; tm_mday; tm_hour; tm_min; tm_sec;_}  =
        Unix.gmtime now in
      Printf.sprintf "%04d%02d%02dT%02d%02d%02dZ" (tm_year + 1900)
    ...
    let fromEnvironment () =
      ...
      | _ ->
          Lwt.fail
            ...
            [@explicit_arity ])
    let fromIni () = object (this)  end
  end
type config =
  {
  resolveAuth: unit -> Auth.t Lwt.t ;
  resolveRegion: unit -> string }
let defaultConfig () =
  let auth = Auth.fromEnvironment () in
  let region =
    (Sys.getenv_opt "AWS_DEFAULT_REGION") |>
      (Option.value ~default:"us-east-1") in
  let resolveRegion () = region in
  let resolveAuth () = auth in { resolveRegion; resolveAuth }
module Service =
  struct
    type descriptor =
      {
    ...
    let makeUri config (service : descriptor) =
      Uri.make ~scheme:"https"
        ~host:(Printf.sprintf "%s.%s.amazonaws.com" service.endpointPrefix
                 (config.resolveRegion ())) ()
  end
module Sign =
  struct
    module SHA256 = Digestif.SHA256
    let sign_request ~auth:(auth : Auth.t)  ~region:(region : string) 
      ~service:(service : Service.descriptor)  ~uri:(uri : Uri.t) 
      ~method_:(method_ : Http.M.method_) 
      ~headers:(headers : (string * string) list)  ~body:(body : string)  =
      let httpRequestMethod =
        (Http.M.string_of_method method_) |> String.uppercase_ascii in
      let canonicalUri =
        ((((Uri.path uri) |> (Str.split (Str.regexp "/"))) |>
            (List.map (fun str -> Uri.pct_encode str)))
           |> (String.concat "/"))
          |>
          (fun str ->
             match String.equal str "" with
             | true -> "/"
             | false -> "/" ^ (str ^ "/")) in
      ...
      let canonicalHeaders =
        ((((extendedHeaders |>
              (List.map
                 (fun (key, value) -> ((String.lowercase_ascii key), value))))
             |>
             (List.sort
                (fun (key1, _) -> fun (key2, _) -> String.compare key1 key2)))
      ...
      let signedHeaders =
        ((extendedHeaders |>
          ...
          |> (String.concat ";") in
      let hash =
        ((SHA256.digest_string body) |> SHA256.to_hex) |>
      ...
      (let canonicalRequestHash =
       ...
       let credentialScope =
         date ^
           ("/" ^ (region ^ ("/" ^ (service.namespace ^ "/aws4_request")))) in
       let stringToSign =
         algorithm ^
           ("\n" ^
              (xAmzDate ^
                 ("\n" ^ (credentialScope ^ ("\n" ^ canonicalRequestHash))))) in
       let kSecret = auth.secretAccessKey in
       let kDate = SHA256.hmac_string ~key:("AWS4" ^ kSecret) date in
       let kRegion =
         SHA256.hmac_string ~key:(SHA256.to_raw_string kDate) region in
       let kService =
         SHA256.hmac_string ~key:(SHA256.to_raw_string kRegion)
           service.namespace in
       let kSigning =
         SHA256.hmac_string ~key:(SHA256.to_raw_string kService)
           "aws4_request" in
       let signature =
         ((SHA256.hmac_string ~key:(SHA256.to_raw_string kSigning)
             stringToSign)
            |> SHA256.to_hex)
           |> String.lowercase_ascii in
       let authorization =
         Printf.sprintf "%s Credential=%s/%s, SignedHeaders=%s, Signature=%s"
           algorithm auth.accessKeyId credentialScope signedHeaders signature in
