...
module Auth = struct
  ...
  let load_ini_file_res file_path =
    ...
    with Eio.Exn.Io _ as ex ->
      Error
        (AuthError
           (Fmt.str "Unable to load ~/.aws/credentials: %a" Eio.Exn.pp ex))

  let load_credentials_profiles env =
    let home_dir = Sys.getenv "HOME" in
    load_ini_file_res
      Eio.Path.(Eio.Stdenv.fs env / home_dir / ".aws" / "credentials")

  let load_config_profiles env =
    let home_dir = Sys.getenv "HOME" in
    load_ini_file_res
      Eio.Path.(Eio.Stdenv.fs env / home_dir / ".aws" / "config")
    |> Result.map (fun kvps ->
           List.filter_map
    ...
    | Some profile_values -> (
        ...
        | _ ->
            raise
              (AuthError
                 (Fmt.str
                    "Profile '%s' missing aws_access_key_id or \
  ...
  let fromProfile env profile_name =
    try
      ...
      let profiles =
        ...
        | Ok config_file, Ok credentials_file ->
            List.concat [ config_file; credentials_file ]
    ...
    with Not_found ->
      raise
        (AuthError "Unable to find $HOME in environment for loading AWS profile")
end
