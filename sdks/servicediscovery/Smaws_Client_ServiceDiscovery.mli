...
(**{|
    ...
    health check, Cloud Map associates the health check with the records that
    ...
    see {{: http://aws.amazon.com/route53/pricing/ }Amazon Route 53
    Pricing}.
    ...
    address is unhealthy, Route 53 considers both the [A] and [AAAA] records
    to be unhealthy.
...
type health_check_config = {
  failure_threshold: int option;
   (**{|
    ...
    Route 53 Determines Whether an Endpoint Is Healthy} in the {i Route 53
    Developer Guide}.|}*)
  ...
  resource_path: string option;
   (**{|
    ...
    If you specify [TCP] for [Type], you must {i not} specify a value for
    [ResourcePath].|}*)
...
type dns_config = {
  ...
  routing_policy: routing_policy option;
   (**{|
    ...
    Answer Routing} in the {i Route 53 Developer Guide}.
    ...
    service. Currently, all records have the same weight, so you can't route
    ...
    healthy, Route 53 responds to DNS queries as if all of the instances
    were healthy.
    ...
    Routing} in the {i Route 53 Developer Guide}.
    |}*)
...
(**{|
    ...
    {ol 
          {- You can't use a health check that's defined by
             [HealthCheckConfig] because the resource isn't available over
             the internet. For example, you can use a custom health check
             when the instance is in an Amazon VPC. (To check the health of
    ...
    {ol 
          ...
          {- You configure a third-party health checker to monitor the
             resource that's associated with the new instance.
...
type health_check_custom_config = {
  failure_threshold: int option;
   (**{|
    ...
    with the same value before 30 seconds has passed doesn't accelerate the
    change. Cloud Map still waits [30] seconds after the first request to
    make the change.|}*)
   ...
   (**{|
    ...
    {ol 
          ...
          {- If the service that's specified by [ServiceId] includes
             [HealthCheckConfig] settings, Cloud Map will create the
    ...
    AWS_EC2_INSTANCE_ID
    {i HTTP namespaces only.} The Amazon EC2 instance ID for the instance. If
  ...
  instance_id: string;
   (**{|
    ...
    {ol 
          ...
          {- If you specify an existing [InstanceId] and [ServiceId], Cloud
             Map updates the existing DNS records, if any. If there's also an
             existing health check, Cloud Map deletes the old health check
             and creates a new one.
             
          The health check isn't deleted immediately, so it will still appear
          for a while if you submit a [ListHealthChecks] request, for
          example.
          }
...
type operation_summary = {
  status: operation_status option;
   (**{|
    ...
    {ol 
          {- {b SUBMITTED}: This is the initial state immediately after you
             submit a request.
             }
...
type operation_filter = {
   ...
   (**{|
    ...
    {ol 
          {- [EQ]: When you specify [EQ] for the condition, you can specify
             ...
             [SERVICE_ID], [STATUS], and [TYPE]. [EQ] is the default
          ...
          {- [IN]: When you specify [IN] for the condition, you can specify a
             ...
             [TYPE]. An operation must match one of the specified values to
             be returned in the response.
             }
          
          {- [BETWEEN]: Specify a start date and an end date in Unix
             date/time format and Coordinated Universal Time (UTC). The start
             date must be the first value. [BETWEEN] is supported for
             [UPDATE_DATE].
             }
  ...
  values: string list;
   (**{|
    ...
    {ol 
          ...
          {- {b STATUS}: Specify one or more statuses: [SUBMITTED],
          ...
          {- {b UPDATE_DATE}: Specify a start date and an end date in Unix
             date/time format and Coordinated Universal Time (UTC). The start
  ...
  name: operation_filter_name;
   (**{|
    ...
    {ol 
          {- {b NAMESPACE_ID}: Gets operations related to specified
             namespaces.
             }
          ...
          {- {b STATUS}: Gets operations based on the status of the
          ...
          {- {b UPDATE_DATE}: Gets operations that changed status during a
...
type list_operations_request = {
  ...
  max_results: int option;
   (**{|
    ...
    [MaxResults], Cloud Map returns up to 100 operations.|}*)
...
type namespace_filter = {
  condition: filter_condition option;
   (**{|
    ...
    {ol 
          {- [EQ]: When you specify [EQ] for [Condition], you can specify
             only one value. [EQ] is supported for [TYPE], [NAME], and
...
type instance_summary = {
  attributes: (string * string) list option;
   (**{|
    ...
    AWS_INSTANCE_CNAME
    For a [CNAME] record, the domain name that Route 53 returns in response
    ...
    AWS_INSTANCE_IPV6
    For an [AAAA] record, the IPv6 address that Route 53 returns in response
    ...
    addition, if the service includes [HealthCheckConfig], the port on the
...
type operation = {
  ...
  update_date: float option;
   (**{|
    ...
    value, in Unix date/time format and Coordinated Universal Time (UTC). The
    value of [UpdateDate] is accurate to milliseconds. For example, the value
    [1516925490.087] represents Friday, January 26, 2018 12:11:30.087 AM.|}*)

  create_date: float option;
   (**{|
    The date and time that the request was submitted, in Unix date/time
    format and Coordinated Universal Time (UTC). The value of [CreateDate] is
...
type get_instances_health_status_response = {
  next_token: string option;
   (**{|
    If more than [MaxResults] instances match the specified criteria, you can
    submit another [GetInstancesHealthStatus] request to get the next group
    of results. Specify the value of [NextToken] from the previous response
    in the next request.|}*)
...
type instance = {
  attributes: (string * string) list option;
   (**{|
    ...
    that's associated with the load balancer. For information about how to
    get the DNS name, see
  ...
  id: string;
   (**{|
    ...
    {ol 
          ...
          {- If you specify an existing [InstanceId] and [ServiceId], Cloud
             Map updates the existing DNS records. If there's also an
             existing health check, Cloud Map deletes the old health check
             and creates a new one.
...
(**{|
    ...
    number of requests. For more information, see
    {{: https://docs.aws.amazon.com/cloud-map/latest/dg/throttling.html }Cloud
    Map API request throttling quota} in the {i Cloud Map Developer Guide}.|}*)
...
type discover_instances_revision_response = {
  instances_revision: int option;
   (**{|
    ...
    new instance is registered or deregistered, the [InstancesRevision]
...
type discover_instances_request = {
  ...
  optional_parameters: (string * string) list option;
   (**{|
    ...
    are returned. Otherwise, the filters are ignored, and only instances that
    match the filters that are specified in the [QueryParameters] parameter
    are returned.|}*)
  ...
  namespace_name: string;
   (**{|
    The [HttpName] name of the namespace. It's found in the [HttpProperties]
    member of the [Properties] member of the namespace. In most cases, [Name]
    and [HttpName] match. However, if you reuse [Name] for namespace
    creation, a generated hash is added to [HttpName] to distinguish the two.|}*)
...
type create_service_request = {
  type_: service_type_option option;
   (**{|
    ...
    using the [DiscoverInstances] API operation. No DNS records is registered
    for the service instances. The only valid value is [HTTP].|}*)
  ...
  name: string;
   (**{|
    ...
    instance and you're using a system that requires a specific [SRV] format,
    such as {{: http://www.haproxy.org/ }HAProxy}, specify the following for
    [Name]:
