...
type resource_sync_attempt = {
  ...
  target_revision: revision;
   (**{|
    ...
    [config-file] in the linked repository. Git sync attempts to update the
    resource to this state.|}*)
  ...
  status: resource_sync_status;
   (**{|
    ...
    {ol 
          {- SYNC-INITIATED - A resource sync attempt has been created and
             will begin soon.
             }
          
          {- SYNCING - Syncing has started and work is being done to
             reconcile state.
             }
...
type repository_sync_definition = {
  ...
  directory: string;
   (**{|
    ...
    from creating or updating the [config-file] field of a
    [sync-configuration].|}*)
  ...
  status: repository_sync_status;
   (**{|
    ...
    {ol 
          ...
          {- IN_PROGRESS - A repository sync attempt has started and work is
...
type list_repository_sync_definitions_output = {
  ...
  repository_sync_definitions: repository_sync_definition list;
   (**{|
    ...
    [RepositorySyncDefinition] is a mapping from a repository branch to all
    the Amazon Web Services resources that are being synced from that branch.|}*)
...
(**{|
    ...
    installed third-party provider type, such as GitHub Enterprise Server.
...
type list_hosts_input = {
  next_token: string option;
   (**{|
    The token that was returned from the previous [ListHosts] call, which can
...
(**{|
    ...
    in `PENDING` status by default. You can make its status `AVAILABLE` by
    updating the connection in the console.|}*)
type connection = {
  host_arn: string option;
   (**{|
...
type create_connection_output = {
  ...
  connection_arn: string;
   (**{|
    ...
    is used as the connection reference when the connection is shared between
...
(**{|
    ...
    resource that can be given to services such as CodePipeline to connect to
    a third-party repository such as Bitbucket. For example, you can add the
    ...
    {ol 
          {- {{:  }CreateConnection}, which creates a uniquely named
             connection that can be referenced by services such as
             CodePipeline.
             }
          ...
          {- {{:  }ListConnections}, which lists the connections associated
             with your account.
             }
    ...
    {ol 
          ...
          {- {{:  }GetHost}, which returns information about the host,
    ...
    {ol 
          {- {{:  }ListTagsForResource}, which gets information about Amazon
             Web Services tags for a specified Amazon Resource Name (ARN) in
             Amazon Web Services CodeConnections.
             }
          ...
          {- {{:  }UntagResource}, which removes tags for a resource in
             Amazon Web Services CodeConnections.
             }
    ...
    the
    {{: https://docs.aws.amazon.com/dtconsole/latest/userguide/welcome-connections.html }Developer
    Tools User Guide}.|}*)
