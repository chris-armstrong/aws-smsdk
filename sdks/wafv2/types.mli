...
(**{|
    ...
    This is used in the {{:  }FieldToMatch} specification for some web
...
type body = {
  oversize_handling: oversize_handling option;
   (**{|
    ...
    {ol 
          {- For Application Load Balancer and AppSync, the limit is fixed at
             8 KB (8,192 bytes).
             }
    ...
    with your rule and web ACL action settings, so that you block any request
...
type json_match_pattern = {
  included_paths: string list option;
   (**{|
    ...
    documentation {{: https://tools.ietf.org/html/rfc6901 }JavaScript Object
...
(**{|
    ...
    body to inspect using the rule's inspection criteria. WAF inspects only
...
type json_body = {
   ...
   (**{|
    ...
    {ol 
          {- [EVALUATE_AS_STRING] - Inspect the body as plain text. WAF
             applies the text transformations and inspection criteria that
             you defined for the JSON inspection to the body text string.
             }
  ...
  match_scope: json_match_scope;
   (**{|
    ...
    [All] does not require a match to be found in the keys and a match to be
    found in the values. It requires a match to be found in the keys or the
    values or both. To require a match in the keys and in the values, use a
    logical [AND] statement to combine two match rules, one that inspects the
...
type headers = {
  oversize_handling: oversize_handling;
   (**{|
    ...
    {ol 
          {- [CONTINUE] - Inspect the available headers normally, according
...
type cookies = {
  oversize_handling: oversize_handling;
   (**{|
    ...
    {ol 
          {- [CONTINUE] - Inspect the available cookies normally, according
             to the rule inspection criteria.
             }
...
(**{|
    ...
    logs. For information about the logging fields, see
    {{: https://docs.aws.amazon.com/waf/latest/developerguide/logging-fields.html }Log
    fields} in the {i WAF Developer Guide}.
...
(**{|
    ...
    {ol 
          {- In a rule statement, this is the part of the web request that
             you want WAF to inspect. Include the single [FieldToMatch] type
             that you want to inspect, with additional specifications as
             needed, according to the type. You specify a single request
             component in [FieldToMatch] for each rule statement that
             requires it. To inspect more than one component of the web
          ...
          {- In a logging configuration, this is used in the [RedactedFields]
             property to specify a field to redact from the logging records.
             For this use case, note the following:
             
          {ol 
                {- Even though all [FieldToMatch] settings are available, the
                   only valid settings for field redaction are [UriPath],
                   [QueryString], [SingleHeader], and [Method].
                   }
                
                {- In this documentation, the descriptions of the individual
                   fields talk about specifying the web request component to
                   inspect, but for field redaction, you are specifying the
                   component type to redact from the logs.
                   }
                
                {- If you have request sampling enabled, the redacted fields
                   configuration for logging has no impact on sampling. The
                   only way to exclude fields from request sampling is by
                   disabling sampling in the web ACL visibility
                   configuration.
                   }
...
type field_to_match = {
  ...
  cookies: cookies option;
   (**{|
    ...
    content in the [Cookies] object. WAF applies the pattern matching filters
  ...
  headers: headers option;
   (**{|
    ...
    matching filters in the [Headers] object, to define the set of headers to
    ...
    content in the [Headers] object. WAF applies the pattern matching filters
...
type custom_http_header = {
  ...
  name: string;
   (**{|
    ...
    request, it prefixes this name [x-amzn-waf-], to avoid confusion with the
    headers that are already in the request. For example, for the header name
    [sample], WAF inserts the header [x-amzn-waf-sample].|}*)
...
type custom_response = {
  response_headers: custom_http_header list option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/waf/latest/developerguide/limits.html }WAF
  ...
  custom_response_body_key: string option;
   (**{|
    ...
    default web ACL action that is set to block. To do this, you first define
    the response body key and value in the [CustomResponseBodies] setting for
    the {{:  }WebACL} or {{:  }RuleGroup} where you want to use it. Then, in
...
(**{|
    ...
    action doesn't block the request. For example, [CaptchaAction] for
...
(**{|
    In a {{:  }WebACL}, this is the action that you want WAF to perform when
    a web request doesn't match any of the rules in the [WebACL]. The default
...
(**{|
    ...
    the location in requests that you want WAF to search, and other settings.
    The bytes to search for are typically a string that corresponds with
    ASCII characters. In the WAF console and the developer guide, this is
...
type byte_match_statement = {
  positional_constraint: positional_constraint;
   (**{|
    ...
    [SearchString] must be a word, which means that both of the following are
    true:
   ...
   (**{|
    ...
    {ol 
          ...
          {- [JA3Fingerprint]: Available for use with Amazon CloudFront
             ...
             match [ByteMatchStatement] with the [PositionalConstraint] set
             to [EXACTLY].
          ...
          it in the logs. For information about the logging fields, see
          {{: https://docs.aws.amazon.com/waf/latest/developerguide/logging-fields.html }Log
          fields} in the {i WAF Developer Guide}.
          }
          
          {- [HeaderOrder]: The list of header names to match for. WAF
             creates a string that contains the ordered list of header names,
             from the headers in the web request, and then matches against
             that string.
             }
...
type sqli_match_statement = {
  sensitivity_level: sensitivity_level option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/waf/latest/developerguide/web-acl-testing.html }Testing
    and tuning} in the {i WAF Developer Guide}.
...
(**{|
    ...
    statement to block requests that have a larger request body size. For
    more information about the inspection limits, see [Body] and [JsonBody]
    settings for the [FieldToMatch] data type.
...
(**{|
    ...
    {ol 
          {- To manage requests only by country, you can use this statement
             by itself and specify the countries that you want to match
             against in the [CountryCodes] array.
             }
          
          {- Otherwise, configure your geo match rule with Count action so
             that it only labels requests. Then, add one or more label match
             rules to run after the geo match rule and configure them to
             match against the geographic labels and handle the requests as
             needed.
             }
...
type geo_match_statement = {
  ...
  country_codes: country_code list option;
   (**{|
    ...
    for example, [[ "US", "CN" ]], from the alpha-2 country ISO codes of the
    ...
    matching requests, but it will still generate logging and count metrics
    for any matches. You can reduce the logging and metrics that the rule
    produces by specifying a country that's unlikely to be a source of
...
(**{|
    ...
    {ol 
          {- If the request includes a valid, unexpired [CAPTCHA] token, WAF
             ...
             configured and then allows the web request inspection to proceed
             to the next rule, similar to a [CountAction].
             }
          ...
          {ol 
                ...
                {- If the request contains an [Accept] header with a value of
                   [text/html], the response includes a [CAPTCHA] JavaScript
                   page interstitial.
                   }
...
(**{|
    ...
    {ol 
          ...
          {- If the request doesn't include a valid, unexpired challenge
             token, WAF discontinues the web ACL evaluation of the request
             and blocks it from going to its intended destination.
          ...
          JavaScript challenge that inspects the client session for
          legitimate behavior. The challenge blocks bots or at least
  ...
  position: forwarded_ip_position;
   (**{|
    ...
    {ol 
          ...
          {- LAST - Inspect the last IP address in the list of IP addresses
             in the header.
             }
...
(**{|
    ...
    specifies the addresses you want to detect, then use the ARN of that set
...
(**{|
    ...
    you use a single header as your custom key, then each value fully defines
...
(**{|
    ...
    you use a single cookie as your custom key, then each value fully defines
    an aggregation instance.|}*)
...
(**{|
    ...
    argument as your custom key, then each value fully defines an aggregation
    instance.|}*)
...
(**{|
    ...
    rule. Each distinct string contributes to the aggregation instance. If
    you use just the query string as your custom key, then each string fully
...
(**{|
    ...
    JSON specification for using the forwarded IP address doesn't explicitly
    use this data type.
...
type rate_based_statement_custom_key = {
  uri_path: rate_limit_uri_path option;
   (**{|
    ...
    your custom key, then each URI path fully defines an aggregation
    instance.|}*)
  ...
  query_string: rate_limit_query_string option;
   (**{|
    ...
    as your custom key, then each string fully defines an aggregation
    instance.|}*)
   ...
   (**{|
    ...
    {ol 
          {- For JSON payloads, specify the field name in JSON pointer
             ...
             {{: https://tools.ietf.org/html/rfc6901 }JavaScript Object
             Notation (JSON) Pointer}.
...
type aws_managed_rules_bot_control_rule_set = {
  enable_machine_learning: bool option;
   (**{|
    ...
    Bot Control rules [TGT_ML_CoordinatedActivityLow] and
    [TGT_ML_CoordinatedActivityMedium], which inspect for anomalous behavior
...
(**{|
    ...
    configure more than one component for inspection. If you don't configure
...
type aws_managed_rules_atp_rule_set = {
  ...
  response_inspection: response_inspection option;
   (**{|
    ...
    sessions and IP addresses that have had too many failed login attempts in
  ...
  login_path: string;
   (**{|
    ...
    URL [https://example.com/web/login], you would provide the path
  ...
  identifier: string;
   (**{|
    ...
    {ol 
          ...
          "THE_PHONE1", "primaryphoneline2": "THE_PHONE2",
          "primaryphoneline3": "THE_PHONE3" } }], the phone number field
          identifiers are [/form/primaryphoneline1],
...
type aws_managed_rules_acfp_rule_set = {
  ...
  response_inspection: response_inspection option;
   (**{|
    ...
    send back to client account creation attempts, keeping count of
    successful and failed attempts from each IP address and client session.
    Using this information, the rule group labels and mitigates requests from
    client sessions and IP addresses that have had too many successful
    account creation attempts in a short amount of time.|}*)
  ...
  registration_page_path: string;
   (**{|
    ...
    would provide the path [/web/registration]. Registration page paths that
    start with the path that you provide are considered a match. For example
    [/web/registration] matches the registration paths [/web/registration],
    [/web/registration/], [/web/registrationPage], and
  ...
  creation_path: string;
   (**{|
    ...
    the page on your website that accepts the completed registration form for
    a new user. This page must accept [POST] requests.
    
    For example, for the URL [https://example.com/web/newaccount], you would
    provide the path [/web/newaccount]. Account creation page paths that
    start with the path that you provide are considered a match. For example
    [/web/newaccount] matches the account creation paths [/web/newaccount],
    [/web/newaccount/], [/web/newaccountPage], and
...
(**{|
    ...
    {ol 
          {- Use the [AWSManagedRulesACFPRuleSet] configuration object to
             ...
             creation request payload of data, such as the user email and
             phone number fields.
             }
          
          {- Use the [AWSManagedRulesATPRuleSet] configuration object to
             configure the account takeover prevention managed rule group.
             The configuration includes the sign-in page of your application
             and the locations in the login request payload of data such as
             the username and password.
             }
          
          {- Use the [AWSManagedRulesBotControlRuleSet] configuration object
             to configure the protection level that you want the Bot Control
             rule group to use.
             }
...
type managed_rule_group_config = {
  aws_managed_rules_acfp_rule_set: aws_managed_rules_acfp_rule_set option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/waf/latest/developerguide/waf-acfp.html }WAF
    Fraud Control account creation fraud prevention (ACFP)} in the {i WAF
    Developer Guide}.|}*)
  ...
  aws_managed_rules_atp_rule_set: aws_managed_rules_atp_rule_set option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/waf/latest/developerguide/waf-atp.html }WAF
    Fraud Control account takeover prevention (ATP)} in the {i WAF Developer
    Guide}.|}*)
  ...
  aws_managed_rules_bot_control_rule_set: aws_managed_rules_bot_control_rule_set option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/waf/latest/developerguide/waf-bot-control.html }WAF
    Bot Control} in the {i WAF Developer Guide}.|}*)
...
(**{|
    ...
    label match string, WAF performs the search for labels that were added in
   ...
   (**{|
    ...
    {ol 
          {- If the [Scope] indicates [LABEL], then this specification must
             include the name and can include any number of preceding
             namespace specifications and prefix up to providing the fully
             qualified label name.
             }
          
          {- If the [Scope] indicates [NAMESPACE], then this specification
             can include any number of contiguous namespace strings, and can
...
type and_statement = {
  ...
  scope_down_statement: statement option;
   (**{|
    ...
    that are evaluated by the managed rule group. Requests are only evaluated
    by the rule group if they match the scope-down statement. You can use any
  ...
  version: string option;
   (**{|
    ...
    version setting is fixed until you change it. If you don't specify this,
    WAF uses the vendor's default version, and then keeps the version at the
  ...
  aggregate_key_type: rate_based_statement_aggregate_key_type;
   (**{|
    ...
    {ol 
          ...
          {- [CUSTOM_KEYS] - Aggregate the request counts using one or more
  ...
  evaluation_window_sec: int option;
   (**{|
    ...
    immediately preceding the current time. Valid settings are 60, 120, 300,
    and 600.
  ...
  managed_rule_group_statement: managed_rule_group_statement option;
   (**{|
    ...
    group. To use this, provide the vendor name and the name of the rule
    group in this statement. You can retrieve the required names by calling
    {{:  }ListAvailableManagedRuleGroups}.
    
    You cannot nest a [ManagedRuleGroupStatement], for example for use inside
    a [NotStatement] or [OrStatement]. You cannot use a managed rule group
    inside another rule group. You can only reference a managed rule group as
    a top-level statement within a rule that you define in a web ACL.
    ...
    For more information, see {{: http://aws.amazon.com/waf/pricing/ }WAF
    Pricing}.
    |}*)
   ...
   (**{|
    ...
    {ol 
          ...
          {- If the aggregation criteria is HTTP method, then each individual
             HTTP method is an aggregation instance. The aggregation
             instances and request counts for our example would be the
             following:
    ...
    limiting web requests using rate-based rules} in the {i WAF Developer
    Guide}.
...
(**{|
    ...
    multiple contexts, for example, in [RuleLabels] inside a {{:  }Rule} and
...
type visibility_config = {
  metric_name: string;
   (**{|
    ...
    name can be from one to 128 characters long. It can't contain whitespace
    or metric names that are reserved for WAF, for example [All] and
    [Default_Action].|}*)
  ...
  cloud_watch_metrics_enabled: bool;
   (**{|
    ...
    that pass the inspection of all rules in the web ACL without being either
    allowed or blocked. For more information, see
...
(**{|
    ...
    to identify web requests that you want to manage in some way. Each rule
    includes one top-level {{:  }Statement} that WAF uses to identify
...
type rule = {
  ...
  captcha_config: captcha_config option;
   (**{|
    ...
    specify this, WAF uses the [CAPTCHA] configuration that's defined for the
    web ACL.|}*)
  ...
  visibility_config: visibility_config;
   (**{|
    ...
    If you change the name of a [Rule] after you create it and you want the
    rule's metric name to reflect the change, update the metric name as well.
  ...
  name: string;
   (**{|
    ...
    If you change the name of a [Rule] after you create it and you want the
    rule's metric name to reflect the change, update the metric name in the
    rule's [VisibilityConfig] settings. WAF doesn't automatically update the
...
(**{|
    ...
    regular rule {{:  }Statement}, but it can only contain a single rule
    group reference.|}*)
...
type firewall_manager_rule_group = {
  ...
  priority: int;
   (**{|
    ...
    in order, starting from the lowest priority setting. The priorities don't
...
type request_body_associated_resource_type_config = {
  default_size_inspection_limit: size_inspection_limit;
   (**{|
    ...
    Verified Access resource should send to WAF for inspection. This applies
    to statements in the web ACL that inspect the body or JSON body.
...
(**{|
    ...
    a web ACL can be a combination of the types {{:  }Rule},
    {{:  }RuleGroup}, and managed rule group. You can associate a web ACL
    with one or more Amazon Web Services resources to protect. The resources
    can be an Amazon CloudFront distribution, an Amazon API Gateway REST API,
...
type web_ac_l = {
  ...
  token_domains: string list option;
   (**{|
    ...
    domains, WAF accepts tokens only for the domain of the protected
  ...
  captcha_config: captcha_config option;
   (**{|
    ...
    don't have their own [CaptchaConfig] settings. If you don't specify this,
    WAF uses its default settings for [CaptchaConfig].|}*)
  ...
  label_namespace: string option;
   (**{|
    ...
    {ol 
          ...
          {- When a rule with a label matches a web request, WAF adds the
  ...
  managed_by_firewall_manager: bool option;
   (**{|
    ...
    then only Firewall Manager can delete the web ACL or any Firewall Manager
  ...
  rules: rule list option;
   (**{|
    ...
    want to manage. Each rule includes one top-level statement that WAF uses
    to identify matching web requests, and parameters that govern how WAF
    handles them.|}*)
...
(**{|
    ...
    you might just need to wait a few minutes. It can take from a few seconds
    to a number of minutes for changes to propagate. Verify the resources
    that you are specifying in your request parameters and then retry the
    operation.|}*)
...
(**{|
    ...
    or more. If you recently made a call to [DeleteServiceLinkedRole], wait
...
(**{|
    ...
    web ACL traffic information} in the {i WAF Developer Guide}.|}*)
...
(**{|
    ...
    {ol 
          ...
          {- Your nested statement isn't valid. You might have tried to nest
             a statement that can’t be nested.
             }
...
(**{|
    ...
    headers, or cookies without specifying how to handle oversize components.
    Rules that inspect the body must either provide an [OversizeHandling]
    configuration or they must be preceded by a [SizeConstraintStatement]
    that blocks the body content from being too large. Rules that inspect the
    headers or cookies must provide an [OversizeHandling] configuration.
    
    Provide the handling configuration and retry your operation.
    
    Alternately, you can suppress this warning by adding the following tag to
    the resource that you provide to this operation: [Tag]
...
(**{|
    ...
    of web requests. WAF gets the specified number of requests from among the
    first 5,000 requests that your Amazon Web Services resource receives
...
(**{|
    ...
    purposes like billing or other management. Typically, the tag key
...
(**{|
    Part of the response from {{:  }GetSampledRequests}. This is a complex
    type that appears as [Headers] in the response syntax. [HTTPHeader]
...
(**{|
    Part of the response from {{:  }GetSampledRequests}. This is a complex
    type that appears as [Request] in the response syntax. [HTTPRequest]
...
type sampled_http_request = {
  ...
  labels: label list option;
   (**{|
    ...
    concatenation of a label namespace and a rule label. The rule's rule
  ...
  rule_name_within_rule_group: string option;
   (**{|
    ...
    the format for this name is [##]. For your own rule groups, the format
    for this name is [#]. If the rule is not in a rule group, this field is
    absent.|}*)
  ...
  weight: int;
   (**{|
    ...
    that has a weight of [2] represents roughly twice as many web requests as
...
(**{|
    ...
    {{:  }DescribeManagedRuleGroup}. This provides information like the ID,
    that you can use to retrieve and manage a [RuleGroup], and the ARN, that
    you provide to the {{:  }RuleGroupReferenceStatement} to use the rule
    group in a {{:  }Rule}.|}*)
type rule_summary = {
  action: rule_action option;
   (**{|
...
(**{|
    ...
    like create and list. This provides information like the ID, that you can
    use to retrieve and manage a [RuleGroup], and the ARN, that you provide
...
(**{|
    ...
    requests that you can use in a {{:  }WebACL}. When you create a rule
    group, you define an immutable capacity limit. If you update a rule
    group, you must stay within the capacity. This allows others to reuse the
...
type rule_group = {
  ...
  label_namespace: string option;
   (**{|
    ...
    {ol 
          {- The syntax for the label namespace prefix for your rule groups
             is the following:
...
(**{|
    ...
    operations like create and list. This provides information like the ID,
    that you can use to retrieve and manage a [RegexPatternSet], and the ARN,
    that you provide to the {{:  }RegexPatternSetReferenceStatement} to use
    the pattern set in a {{:  }Rule}.|}*)
...
(**{|
    ...
    {{:  }RateBasedStatement}. This is only available for rate-based rules
    ...
    scope-down statement. When a rule has no scope-down statement, it applies
    the action to all requests from the IP addresses that are in the list.
    The rule applies its rule action to rate limit the matching requests. The
    action is usually Block but it can be any valid rule action except for
    Allow.
...
type action_condition = {
  action: action_value;
   (**{|
    ...
    you've applied a rule action override to the rule, it's the override
    action. The value [EXCLUDED_AS_COUNT] matches on excluded rules and also
...
(**{|
    ...
    {ol 
          {- Create your logging destination. You can use an Amazon
             CloudWatch Logs log group, an Amazon Simple Storage Service
          ...
          {{: https://docs.aws.amazon.com/waf/latest/developerguide/logging.html }Logging
          web ACL traffic} in the {i WAF Developer Guide}.
          }
    ...
    bucket, WAF creates a bucket policy. For an Amazon Kinesis Data Firehose,
    WAF creates a service-linked role.
...
type logging_configuration = {
  log_scope: log_scope option;
   (**{|
    ...
    management. For information, see
    {{: https://docs.aws.amazon.com/security-lake/latest/userguide/internal-sources.html }Collecting
    data from Amazon Web Services services} in the {i Amazon Security Lake
    user guide}.
  ...
  redacted_fields: field_to_match list option;
   (**{|
    ...
    [FieldToMatch] setting, so the [SingleHeader] redaction doesn't apply to
    rules that use the [Headers] [FieldToMatch].
    ...
    the only way to exclude fields is by disabling sampling in the web ACL
...
type managed_rule_set_summary = {
  label_namespace: string option;
   (**{|
    ...
    {ol 
          {- The syntax for the label namespace prefix for a managed rule
...
(**{|
    ...
    {{:  }ManagedRuleGroupStatement} to a web ACL. Managed rule groups
    include Amazon Web Services Managed Rules rule groups and Amazon Web
    Services Marketplace managed rule groups. To use any Amazon Web Services
    Marketplace managed rule group, first subscribe to the rule group through
...
type managed_product_descriptor = {
  is_advanced_managed_rule_set: bool option;
   (**{|
    ...
    such as the the Amazon Web Services Managed Rules rule groups that are
    used for WAF intelligent threat mitigation.|}*)
  ...
  sns_topic_arn: string option;
   (**{|
    ...
    when the managed rule group is modified, such as for new versions and for
    version expiration. For more information, see the
    {{: https://docs.aws.amazon.com/sns/latest/dg/welcome.html }Amazon Simple
...
type list_web_ac_ls_response = {
  web_ac_ls: web_acl_summary list option;
   (**{|
    Array of web ACLs. If you specified a [Limit] in your request, this might
...
(**{|
    ...
    rule, you provide the ARN to the {{:  }Rule} statement
...
type get_web_acl_response = {
  application_integration_ur_l: string option;
   (**{|
    ...
    rule group in your web ACL that integrates with your applications in this
...
type get_sampled_requests_response = {
  time_window: time_window option;
   (**{|
    ...
    specified in the request, [GetSampledRequests] returns the time range for
  ...
  population_size: int option;
   (**{|
    ...
    sample includes every request that your Amazon Web Services resource
...
type get_sampled_requests_request = {
  ...
  time_window: time_window;
   (**{|
    ...
    specify a start time that's earlier than three hours ago, WAF sets it to
    three hours ago.|}*)
...
(**{|
    ...
    "V2" or "v2", to distinguish from the prior version. We recommend
    ...
    API, Amazon Cognito user pool, App Runner service, or Amazon Web Services
    ...
    {{: https://docs.aws.amazon.com/waf/latest/developerguide/what-is-aws-waf.html }WAF
    Developer Guide}.
    
    You can make calls using the endpoints listed in
    {{: https://docs.aws.amazon.com/general/latest/gr/waf.html }WAF endpoints
    and quotas}.
    
    {ol 
          {- For regional applications, you can use any of the endpoints in
             the list. A regional application can be an Application Load
             Balancer (ALB), an Amazon API Gateway REST API, an AppSync
             GraphQL API, an Amazon Cognito user pool, an App Runner service,
             or an Amazon Web Services Verified Access instance.
             }
          
          {- For Amazon CloudFront applications, you must use the API
    ...
    {ol 
          {- You use one API for both global and regional applications. Where
             you need to distinguish the scope, you specify a [Scope]
             parameter and set it to [CLOUDFRONT] or [REGIONAL].
             }
          
          {- You can define a web ACL or rule group with a single call, and
             update it with a single call. You define all rule specifications
             in JSON format, and pass them to your rule group or web ACL
             calls.
             }
          
          {- The limits WAF places on the use of rules more closely reflects
             the cost of running each type of rule. Rule groups include
