...
(**{|
    ...
    {ol 
          {- The rate of requests per second exceeds the Amazon Web Services
             KMS request quota for an account and Region.
             }
          
          {- A burst or sustained high rate of requests to change the state
             of the same KMS key. This condition is often known as a "hot
             key."
             }
          
          {- Requests for operations on KMS keys in a Amazon Web Services
             CloudHSM key store might be throttled at a lower-than-expected
             rate when the Amazon Web Services CloudHSM cluster associated
             with the Amazon Web Services CloudHSM key store is processing
             numerous commands, including those unrelated to the Amazon Web
   ...
   (**{|
    ...
    {ol 
          {- [DelaySeconds] – The length of time, in seconds, for which the
             ...
             An integer from 0 to 900 (15 minutes). Default: 0.
             }
          ...
          {- [MessageRetentionPeriod] – The length of time, in seconds, for
             ...
             the change can take up to 60 seconds for most of the attributes
             to propagate throughout the Amazon SQS system. Changes made to
             the [MessageRetentionPeriod] attribute can take up to 15 minutes
             and will impact existing messages in the queue potentially
          ...
          {- [Policy] – The queue's policy. A valid Amazon Web Services
             ...
             {{: https://docs.aws.amazon.com/IAM/latest/UserGuide/PoliciesOverview.html }Overview
             of Amazon Web Services IAM Policies} in the {i Identity and
             Access Management User Guide}.
             }
          
          {- [ReceiveMessageWaitTimeSeconds] – The length of time, in
             seconds, for which a [{{:  }ReceiveMessage}] action waits for a
    ...
    {ol 
          {- [RedrivePolicy] – The string that includes the parameters for
             the dead-letter queue functionality of the source queue as a
             JSON object. The parameters are as follows:
             
          {ol 
                {- [deadLetterTargetArn] – The Amazon Resource Name (ARN)
                   of the dead-letter queue to which Amazon SQS moves
                ...
                {- [maxReceiveCount] – The number of times a message is
                   ...
                   dead-letter queue. Default: 10. When the [ReceiveCount]
                   for a message exceeds the [maxReceiveCount] for a queue,
          ...
          {- [RedriveAllowPolicy] – The string that includes the parameters
             for the permissions for the dead-letter queue redrive permission
             and which source queues can specify dead-letter queues as a JSON
          ...
          {ol 
                ...
                {ol 
                      {- [allowAll] – (Default) Any source queues in this
                         Amazon Web Services account in the same Region can
                      ...
                      {- [byQueue] – Only queues specified by the
                         [sourceQueueArns] parameter can specify this queue
                ...
                {- [sourceQueueArns] – The Amazon Resource Names (ARN)s of
                   the source queues that can specify this queue as the
                   dead-letter queue and redrive messages. You can specify
                   this parameter only when the [redrivePermission] parameter
                   is set to [byQueue]. You can specify up to 10 source queue
                   ARNs. To allow more than 10 source queues to specify
                   dead-letter queues, set the [redrivePermission] parameter
                   to [allowAll].
                   }
    ...
    {ol 
          {- [KmsMasterKeyId] – The ID of an Amazon Web Services managed
             ...
             always [alias/aws/sqs], the alias of a custom CMK can, for
             example, be [alias/{i MyAlias}]. For more examples, see
          ...
          {- [KmsDataKeyReusePeriodSeconds] – The length of time, in
             ...
             86,400 seconds (24 hours). Default: 300 (5 minutes). A shorter
             time period provides better security but results in more calls
             to KMS which might incur charges after Free Tier. For more
             information, see
          ...
          {- [SqsManagedSseEnabled] – Enables server-side queue encryption
             ...
             option is supported per queue (for example,
    ...
    {ol 
          {- [ContentBasedDeduplication] – Enables content-based
             ...
             processing} in the {i Amazon SQS Developer Guide}. Note the
             following:
          ...
          {ol 
                ...
                {- When [ContentBasedDeduplication] is in effect, messages
                   with identical content sent within the deduplication
                   interval are treated as duplicates and only one copy of
                ...
                {- If you send one message with [ContentBasedDeduplication]
                   ...
                   messages are treated as duplicates and only one copy of
                   the message is delivered.
                   }
    ...
    {ol 
          ...
          {- [FifoThroughputLimit] – Specifies whether the FIFO queue
             throughput quota applies to the entire queue or per message
             group. Valid values are [perQueue] and [perMessageGroupId]. The
...
type message_attribute_value = {
  ...
  string_value: string option;
   (**{|
    ...
    values, see
    {{: http://en.wikipedia.org/wiki/ASCII#ASCII_printable_characters }ASCII
...
type send_message_request = {
  ...
  message_deduplication_id: string option;
   (**{|
    ...
    the MessageDeduplicationId Property} in the {i Amazon SQS Developer
    Guide}.|}*)
   ...
   (**{|
    ...
    {ol 
          {- Currently, the only supported message system attribute is
             [AWSTraceHeader]. Its type must be [String] and its value must
             be a correctly formatted X-Ray trace header string.
             }
  ...
  message_body: string;
   (**{|
    ...
    Unicode characters are allowed. For more information, see the
    {{: http://www.w3.org/TR/REC-xml/#charsets }W3C specification for
    characters}.
    ...
    characters with [U+FFFD] before storing the message in the queue, as long
    as the message body contains at least one valid character.
    |}*)
...
type receive_message_request = {
  receive_request_attempt_id: string option;
   (**{|
    ...
    response you receive a generic error, it is possible to retry the same
    ...
    {ol 
          ...
          {- It is possible to retry the [ReceiveMessage] action with the
             same [ReceiveRequestAttemptId] if none of the messages have been
          ...
          messages and therefore process duplicates. Also, if a consumer
    ...
    the ReceiveRequestAttemptId Request Parameter} in the {i Amazon SQS
    Developer Guide}.|}*)
  ...
  wait_time_seconds: int option;
   (**{|
    ...
    returns sooner than [WaitTimeSeconds]. If no messages are available and
    the wait time expires, the call does not return a message list.
    
    To avoid HTTP errors, ensure that the HTTP response timeout for
    [ReceiveMessage] requests is longer than the [WaitTimeSeconds] parameter.
    For example, with the Java SDK, you can set HTTP transport settings using
    the
   ...
   (**{|
    ...
    {ol 
          ...
          {- The name must not start with AWS-reserved prefixes such as
             [AWS.] or [Amazon.] (or any casing variants).
             }
          
          {- The name must not start or end with a period ([.]), and it
             should not have periods in succession ([..]).
             }
   ...
   (**{|
    ...
    {ol 
          ...
          {- [SentTimestamp] – Returns the time the message was sent to the
             queue ({{: http://en.wikipedia.org/wiki/Unix_time }epoch time}
             in milliseconds).
             }
          ...
          {- [MessageGroupId] – Returns the value provided by the producer
             that calls the [{{:  }SendMessage}] action. Messages with the
...
type list_message_move_tasks_result_entry = {
  ...
  max_number_of_messages_per_second: int option;
   (**{|
    The number of messages to be moved per second (the message movement
    rate), if it has been specified in the [StartMessageMoveTask] request. If
    a [MaxNumberOfMessagesPerSecond] has not been specified in the
  ...
  destination_arn: string option;
   (**{|
    ...
    specified in the [StartMessageMoveTask] request, this field value will be
    NULL.|}*)
   ...
   (**{|
    ...
    {ol 
          ...
          {- [ApproximateNumberOfMessagesDelayed] – Returns the approximate
             number of messages in the queue that are delayed and not
          ...
          {- [ApproximateNumberOfMessagesNotVisible] – Returns the
             ...
             considered to be {i in flight} if they have been sent to a
          ...
          {- [MessageRetentionPeriod] – Returns the length of time, in
             seconds, for which Amazon SQS retains a message. When you change
             a queue's attributes, the change can take up to 60 seconds for
             most of the attributes to propagate throughout the Amazon SQS
             system. Changes made to the [MessageRetentionPeriod] attribute
             can take up to 15 minutes and will impact existing messages in
             the queue potentially causing them to be expired and deleted if
             the [MessageRetentionPeriod] is reduced below the age of
             existing messages.
             }
          ...
          {- [ReceiveMessageWaitTimeSeconds] – Returns the length of time,
             in seconds, for which the [ReceiveMessage] action waits for a
             message to arrive.
             }
    ...
    {ol 
          {- [KmsMasterKeyId] – Returns the ID of an Amazon Web Services
             managed customer master key (CMK) for Amazon SQS or a custom
             CMK. For more information, see
             {{: https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-server-side-encryption.html#sqs-sse-key-terms }Key
             Terms}.
             }
          
          {- [KmsDataKeyReusePeriodSeconds] – Returns the length of time,
             in seconds, for which Amazon SQS can reuse a data key to encrypt
             or decrypt messages before calling KMS again. For more
             information, see
             {{: https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-server-side-encryption.html#sqs-how-does-the-data-key-reuse-period-work }How
             Does the Data Key Reuse Period Work?}.
             }
          ...
          {- [SqsManagedSseEnabled] – Returns information about whether the
             queue is using SSE-SQS encryption using SQS owned encryption
    ...
    {ol 
          ...
          {- [ContentBasedDeduplication] – Returns whether content-based
             deduplication is enabled for the queue. For more information,
             see
             {{: https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/FIFO-queues-exactly-once-processing.html }Exactly-once
             processing} in the {i Amazon SQS Developer Guide}.
             }
...
type create_queue_request = {
  tags: (string * string) list option;
   (**{|
    ...
    cross-account permissions to a role and a username} in the {i Amazon SQS
    Developer Guide}.
    |}*)
  ...
  attributes: (string * string) list option;
   (**{|
    ...
    {ol 
          {- [DelaySeconds] – The length of time, in seconds, for which the
             delivery of all messages in the queue is delayed. Valid values:
             An integer from 0 to 900 seconds (15 minutes). Default: 0.
             }
          
          {- [MaximumMessageSize] – The limit of how many bytes a message
             can contain before Amazon SQS rejects it. Valid values: An
             integer from 1,024 bytes (1 KiB) to 262,144 bytes (256 KiB).
             Default: 262,144 (256 KiB).
             }
          
          {- [MessageRetentionPeriod] – The length of time, in seconds, for
             which Amazon SQS retains a message. Valid values: An integer
             from 60 seconds (1 minute) to 1,209,600 seconds (14 days).
             Default: 345,600 (4 days). When you change a queue's attributes,
             the change can take up to 60 seconds for most of the attributes
             to propagate throughout the Amazon SQS system. Changes made to
             the [MessageRetentionPeriod] attribute can take up to 15 minutes
          ...
          {- [Policy] – The queue's policy. A valid Amazon Web Services
             policy. For more information about policy structure, see
             {{: https://docs.aws.amazon.com/IAM/latest/UserGuide/PoliciesOverview.html }Overview
             of Amazon Web Services IAM Policies} in the {i IAM User Guide}.
             }
    ...
    {ol 
          {- [KmsMasterKeyId] – The ID of an Amazon Web Services managed
             ...
             more information, see
             {{: https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-server-side-encryption.html#sqs-sse-key-terms }Key
             Terms}. While the alias of the Amazon Web Services managed CMK
             for Amazon SQS is always [alias/aws/sqs], the alias of a custom
             CMK can, for example, be [alias/{i MyAlias}]. For more examples,
             see
             {{: https://docs.aws.amazon.com/kms/latest/APIReference/API_DescribeKey.html#API_DescribeKey_RequestParameters }KeyId}
             in the {i Key Management Service API Reference}.
             }
    ...
    {ol 
          {- [FifoQueue] – Designates a queue as FIFO. Valid values are
             [true] and [false]. If you don't specify the [FifoQueue]
             attribute, Amazon SQS creates a standard queue. You can provide
             this attribute only during queue creation. You can't change it
          ...
          {- [ContentBasedDeduplication] – Enables content-based
             deduplication. Valid values are [true] and [false]. For more
             information, see
             {{: https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/FIFO-queues-exactly-once-processing.html }Exactly-once
             processing} in the {i Amazon SQS Developer Guide}. Note the
             following:
...
type add_permission_request = {
  actions: string list;
   (**{|
    ...
    Resource} in the {i Amazon SQS Developer Guide}.
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-authentication-and-access-control.html }Identity
    and access management} in the {i Amazon SQS Developer Guide.}
    
    You can use {{: http://aws.amazon.com/tools/#sdk }Amazon Web Services
    SDKs} to access Amazon SQS using your favorite programming language. The
    ...
    {ol 
          ...
          {ol 
                {- {{: https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-making-api-requests.html }Making
                   API Requests}
                   }
                ...
                {- {{: https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-dead-letter-queues.html }Amazon
          ...
          {- {{: http://docs.aws.amazon.com/cli/latest/reference/sqs/index.html }Amazon
             SQS in the {i Command Line Interface}}
             }
          
          {- {i Amazon Web Services General Reference}
             
          {ol 
                {- {{: https://docs.aws.amazon.com/general/latest/gr/rande.html#sqs_region }Regions
                   and Endpoints}
                   }
