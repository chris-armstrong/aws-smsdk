...
(**{|
    ...
    able to auto-respond to requests or have at least one delegate associated
    that can do so on its behalf.|}*)
...
(**{|
    Filtering options for {i ListUsers} operation. This is only used as input
    to Operation.|}*)
...
type mobile_device_access_rule = {
  ...
  not_device_operating_systems: string list option;
   (**{|
    Device operating systems that a rule {b will not} match. All other device
    types will match.|}*)
...
type get_mobile_device_access_effect_response = {
  ...
  effect_: mobile_device_access_rule_effect option;
   (**{|
    ...
    mobile device access rules in the WorkMail organization for the simulated
    user parameters.|}*)
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/workmail/latest/adminguide/add_domain.html }Adding
    a domain} in the {i WorkMail Administrator Guide}.|}*)
type domain = {
...
type create_availability_configuration_request = {
  ...
  ews_provider: ews_availability_provider option;
   (**{|
    ...
    must contain exactly one provider definition, either [EwsProvider] or
    [LambdaProvider].|}*)
...
(**{|
    ...
    support for existing desktop and mobile email clients. You can access
    your email, contacts, and calendars using Microsoft Outlook, your
    ...
    entire WorkMail organization (or as set in the IAM policy). This
