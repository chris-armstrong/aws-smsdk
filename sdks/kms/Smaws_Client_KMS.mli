...
(**{|
    ...
    store. To identify the cause, see the error message that accompanies the
    exception and
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/vpc-connectivity.html#xks-vpc-requirements }review
    the requirements} for Amazon VPC endpoint service connectivity for an
    external key store.|}*)
...
(**{|
    ...
    Amazon Web Services Region. Each external key store in a Region must use
    a different Amazon VPC endpoint service.|}*)
...
(**{|
    ...
    reachable before you create the external key store or update its
    settings.
...
(**{|
    ...
    external key store in a Region must use a unique external key store proxy
    API address.|}*)
...
(**{|
    ...
    Region. To identify the cause, see the error message that accompanies the
    exception.|}*)
...
(**{|
    ...
    proxy. The problem might be a poorly constructed response, but it could
    also be a transient network issue. If you see this error repeatedly,
    report it to the proxy vendor.|}*)
...
type xks_proxy_configuration_type = {
  ...
  access_key_id: string option;
   (**{|
    ...
    authentication credential} that uniquely identifies the secret access
    key.|}*)
...
(**{|
    ...
    identify the key. For details, see the documentation provided with your
...
(**{|
    ...
    This element appears in a {{:  }CreateKey} or {{:  }DescribeKey} response
    ...
    key in an external key store in a cryptographic operation, the
...
(**{|
    ...
    with another KMS key in this external key store. Each KMS key in an
...
type verify_request = {
  dry_run: bool option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/programming-dryrun.html }Testing
    your KMS API calls} in the {i Key Management Service Developer Guide}.|}*)
  ...
  grant_tokens: string list option;
   (**{|
    ...
    a new grant that has not yet achieved {i eventual consistency}. For more
    information, see
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/grants.html#grant_token }Grant
    token} and
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/grant-manage.html#using-grant-token }Using
    a grant token} in the {i Key Management Service Developer Guide}.|}*)
  ...
  message_type: message_type option;
   (**{|
    ...
    algorithm, which begins with a hash function. When the value is [DIGEST],
    KMS skips the hashing step in the signing algorithm.
    
    Use the [DIGEST] value only when the value of the [Message] parameter is
    a message digest. If you use the [DIGEST] value with an unhashed message,
    ...
    {ol 
          ...
          {- SM2DSA uses the SM3 hashing algorithm. For details, see
             {{: https://docs.aws.amazon.com/kms/latest/developerguide/asymmetric-key-specs.html#key-spec-sm-offline-verification }Offline
  ...
  message: bytes;
   (**{|
    ...
    use the [MessageType] parameter with a value of [DIGEST].
  ...
  key_id: string;
   (**{|
    ...
    When using an alias name, prefix it with ["alias/"]. To specify a KMS key
    in a different Amazon Web Services account, you must use the key ARN or
    alias ARN.
...
type verify_mac_request = {
  ...
  mac: bytes;
   (**{|
    ...
    {{:  }GenerateMac} operation when you specified the same message, HMAC
    KMS key, and MAC algorithm as the values specified in this request.|}*)
  ...
  message: bytes;
   (**{|
    ...
    message digests. If you generated an HMAC for a hash digest of a message,
    you must verify the HMAC for the same hash digest.|}*)
...
(**{|
    ...
    {ol 
          {- The key state of the KMS key is not compatible with the
             operation.
             
          To find the key state, use the {{:  }DescribeKey} operation. For
          more information about which key states are compatible with each
          KMS operation, see
          {{: https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html }Key
          states of KMS keys} in the {i {i Key Management Service Developer
          Guide}}.
          }
          
          {- For cryptographic operations on KMS keys in custom key stores,
             this exception represents a general failure with many possible
             causes. To identify the cause, see the error message that
             accompanies the exception.
             }
...
(**{|
    ...
    verification fails when the HMAC computed by using the specified message,
    HMAC KMS key, and MAC algorithm does not match the HMAC specified in the
    request.|}*)
...
(**{|
    ...
    [KeyUsage] must be [ENCRYPT_DECRYPT]. For signing and verifying messages,
    the [KeyUsage] must be [SIGN_VERIFY]. For generating and verifying
    message authentication codes (MACs), the [KeyUsage] must be
    [GENERATE_VERIFY_MAC]. For deriving key agreement secrets, the [KeyUsage]
    must be [KEY_AGREEMENT]. To find the [KeyUsage] of a KMS key, use the
...
type update_custom_key_store_request = {
  ...
  cloud_hsm_cluster_id: string option;
   (**{|
    ...
    the requirements} for a cluster associated with a custom key store. To
...
(**{|
    ...
    {ol 
          {- You requested the {{:  }ConnectCustomKeyStore} operation on a
             custom key store with a [ConnectionState] of [DISCONNECTING] or
             [FAILED]. This operation is valid for all other
             [ConnectionState] values. To reconnect a custom key store in a
             [FAILED] state, disconnect it ({{:  }DisconnectCustomKeyStore}),
             then connect it ([ConnectCustomKeyStore]).
             }
          
          {- You requested the {{:  }CreateKey} operation in a custom key
             store that is not connected. This operations is valid only when
             the custom key store [ConnectionState] is [CONNECTED].
             }
          
          {- You requested the {{:  }DisconnectCustomKeyStore} operation on a
             custom key store with a [ConnectionState] of [DISCONNECTING] or
          ...
          {- You requested the {{:  }UpdateCustomKeyStore} or
             {{:  }DeleteCustomKeyStore} operation on a custom key store that
             is not disconnected. This operation is valid only when the
             custom key store [ConnectionState] is [DISCONNECTED].
             }
          
          {- You requested the {{:  }GenerateRandom} operation in an CloudHSM
             key store that is not connected. This operation is valid only
             when the CloudHSM key store [ConnectionState] is [CONNECTED].
             }
...
(**{|
    ...
    try the command again. For detailed instructions, see
    {{: https://docs.aws.amazon.com/cloudhsm/latest/userguide/getting-started.html }Getting
    Started} in the {i CloudHSM User Guide}.|}*)
...
(**{|
    ...
    {ol 
          {- The CloudHSM cluster must be configured with private subnets in
             at least two different Availability Zones in the Region.
             }
          
          {- The
             {{: https://docs.aws.amazon.com/cloudhsm/latest/userguide/configure-sg.html }security
             group for the cluster} (cloudhsm-cluster-{i }-sg) must include
             inbound rules and outbound rules that allow TCP traffic on ports
             2223-2225. The {b Source} in the inbound rules and the
             {b Destination} in the outbound rules must match the security
             group ID. These rules are set by default when you create the
             CloudHSM cluster. Do not delete or change them. To get
             information about a particular security group, use the
          ...
          For the {{:  }CreateCustomKeyStore}, {{:  }UpdateCustomKeyStore},
          and {{:  }CreateKey} operations, the CloudHSM cluster must have at
          least two active HSMs, each in a different Availability Zone. For
          the {{:  }ConnectCustomKeyStore} operation, the CloudHSM must
    ...
    the Prerequisites} in the {i Key Management Service Developer Guide}. For
    information about creating a private subnet for an CloudHSM cluster, see
    {{: https://docs.aws.amazon.com/cloudhsm/latest/userguide/create-subnets.html }Create
    a Private Subnet} in the {i CloudHSM User Guide}. For information about
    cluster security groups, see
    {{: https://docs.aws.amazon.com/cloudhsm/latest/userguide/configure-sg.html }Configure
    a Default Security Group} in the {i {i CloudHSM User Guide}}.|}*)
...
type update_alias_request = {
  target_key_id: string;
   (**{|
    ...
    managed key} to associate with the alias. You don't have permission to
    associate an alias with an
...
(**{|
    ...
    Tag Restrictions} in the {i Amazon Web Services Billing and Cost
   ...
   (**{|
    ...
    {ol 
          {- When used with the supported RSA signing algorithms, the
             encoding of this value is defined by
          ...
          {- When used with the [ECDSA_SHA_256], [ECDSA_SHA_384], or
             ...
             {{: https://tools.ietf.org/html/rfc3279#section-2.2.3 }RFC 3279
             Section 2.2.3}. This is the most commonly used signature format
             and is appropriate for most uses.
             }
...
type schedule_key_deletion_response = {
  ...
  key_state: key_state option;
   (**{|
    ...
    see
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html }Key
    states of KMS keys} in the {i Key Management Service Developer Guide}.|}*)
...
type schedule_key_deletion_request = {
  pending_window_in_days: int option;
   (**{|
    ...
    use the
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/conditions-kms.html#conditions-kms-schedule-key-deletion-pending-window-in-days }[kms:ScheduleKeyDeletionPendingWindowInDays]}
...
type rotate_key_on_demand_request = {
  key_id: string;
   (**{|
    ...
    KMS keys},
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/hmac.html }HMAC
    KMS keys}, KMS keys with
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html }imported
    key material}, or KMS keys in a
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html }custom
    key store}. To perform on-demand rotation of a set of related
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-manage.html#multi-region-rotate }multi-Region
...
type revoke_grant_request = {
  ...
  key_id: string;
   (**{|
    ...
    key ID and key ARN for a KMS key, use {{:  }ListKeys} or
    {{:  }DescribeKey}.
...
type key_metadata = {
  ...
  pending_deletion_window_in_days: int option;
   (**{|
    ...
    deleted. This value is present only when the [KeyState] of the KMS key is
    [PendingReplicaDeletion]. That indicates that the KMS key is the primary
    ...
    for deletion, its deletion date is displayed in the [DeletionDate] field.
    However, when the primary key in a multi-Region key is scheduled for
    deletion, its waiting period doesn't begin until all of its replica keys
    are deleted. This value displays that waiting period. When the last
  ...
  key_manager: key_manager_type option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#kms_keys }KMS
  ...
  cloud_hsm_cluster_id: string option;
   (**{|
    ...
    the KMS key. When you create a KMS key in an CloudHSM
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html }custom
    key store}, KMS creates the key material for the KMS key in the
    associated CloudHSM cluster. This field is present only when the KMS key
  ...
  custom_key_store_id: string option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html }custom
    key store} that contains the KMS key. This field is present only when the
  ...
  origin: origin_type option;
   (**{|
    ...
    material. When this value is [AWS_CLOUDHSM], the key material was created
  ...
  deletion_date: float option;
   (**{|
    ...
    still has replica keys, its key state is [PendingReplicaDeletion] and the
    length of its waiting period is displayed in the
  ...
  key_usage: key_usage_type option;
   (**{|
    The
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#cryptographic-operations }cryptographic
  ...
  arn: string option;
   (**{|
    ...
    Management Service (KMS)} in the Example ARNs section of the {i Amazon
...
type replicate_key_response = {
  ...
  replica_key_metadata: key_metadata option;
   (**{|
    ...
    ARN}) and
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html }Key
    states of KMS keys}. It also includes the ARN and Amazon Web Services
...
type replicate_key_request = {
  tags: tag list option;
   (**{|
    ...
    key. For details, see
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/abac.html }ABAC
    for KMS} in the {i Key Management Service Developer Guide}.
    ...
    see
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/tagging-keys.html }Tagging
    Keys}.|}*)
  ...
  policy: string option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html#key-policy-default }default
    ...
    {ol 
          {- The key policy must allow the calling principal to make a
             subsequent [PutKeyPolicy] request on the KMS key. This reduces
          ...
          {- Each statement in the key policy must contain one or more
             ...
             that I make are not always immediately visible} in the {i Amazon
    ...
    policies in KMS} in the {i Key Management Service Developer Guide}. For
    help writing and formatting a JSON policy document, see the
    {{: https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies.html }IAM
    JSON Policy Reference} in the {i {i Identity and Access Management User
    Guide}}.|}*)
...
(**{|
    From the {{:  }Decrypt} or {{:  }ReEncrypt} operation, the request was
    rejected because the specified ciphertext, or additional authenticated
    data incorporated into the ciphertext, such as the encryption context, is
    corrupted, missing, or otherwise invalid.
    
    From the {{:  }ImportKeyMaterial} operation, the request was rejected
    because KMS could not decrypt the encrypted (wrapped) key material.|}*)
...
type re_encrypt_request = {
  ...
  destination_encryption_algorithm: encryption_algorithm_spec option;
   (**{|
    ...
    after it has decrypted it. The default value, [SYMMETRIC_DEFAULT],
...
(**{|
    ...
    a
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/symm-asymm-concepts.html#symmetric-cmks }symmetric
    KMS key}. Grant constraints are not applied to operations that do not
    ...
    context, use the [kms:EncryptionContext:] and [kms:EncryptionContextKeys]
    conditions in an IAM or key policy. For details, see
...
type list_grants_response = {
  truncated: bool option;
   (**{|
    ...
    pass the value of the [NextMarker] element in this response to the
...
type list_retirable_grants_request = {
  retiring_principal: string;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html }Amazon
    Resource Name (ARN)} of an Amazon Web Services principal. Valid
...
type rotations_list_entry = {
  rotation_type: rotation_type option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html#rotating-keys-enable-disable }automatic
    rotation} or an
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html#rotating-keys-on-demand }on-demand
    rotation}.|}*)
...
(**{|
    ...
    Use {{:  }GetParametersForImport} to get a new import token and public
    key, use the new public key to encrypt the key material, and then try the
    request again.|}*)
...
type import_key_material_request = {
  expiration_model: expiration_model_type option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/en_us/kms/latest/developerguide/importing-keys.html#importing-keys-expiration }Setting
  ...
  valid_to: float option;
   (**{|
    ...
    You cannot change the [ExpirationModel] or [ValidTo] values for the
    current import after the request completes. To change either value, you
    must delete ({{:  }DeleteImportedKeyMaterial}) and reimport the key
    material.|}*)

  encrypted_key_material: bytes;
   (**{|
    The encrypted key material to import. The key material must be encrypted
    under the public wrapping key that {{:  }GetParametersForImport}
  ...
  key_id: string;
   (**{|
    ...
    {{: kms/latest/developerguide/multi-region-keys-overview.html }multi-Region
    key} of any supported type. You cannot perform this operation on a KMS
...
type get_parameters_for_import_response = {
  parameters_valid_to: float option;
   (**{|
    ...
    request and you must send another [GetParametersForImport] request to get
    new ones.|}*)
  ...
  key_id: string option;
   (**{|
    The Amazon Resource Name
    ({{: https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-ARN }key
    ARN}) of the KMS key to use in a subsequent {{:  }ImportKeyMaterial}
...
type get_parameters_for_import_request = {
  ...
  wrapping_algorithm: algorithm_spec;
   (**{|
    ...
    information, see
    {{: kms/latest/developerguide/importing-keys-get-public-key-and-token.html#select-wrapping-algorithm }Select
    ...
    {ol 
          {- {b RSA_AES_KEY_WRAP_SHA_256} — Supported for wrapping RSA and
             ECC key material.
             }
          
          {- {b RSA_AES_KEY_WRAP_SHA_1} — Supported for wrapping RSA and
             ECC key material.
             }
          
          {- {b RSAES_OAEP_SHA_256} — Supported for all types of key
          ...
          {- {b RSAES_OAEP_SHA_1} — Supported for all types of key
             material, except RSA key material (private key).
          ...
          {- {b RSAES_PKCS1_V1_5} (Deprecated) — As of October 10, 2023,
...
type get_key_rotation_status_response = {
  on_demand_rotation_start_date: float option;
   (**{|
    ...
    The KMS API follows an
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/programming-eventual-consistency.html }eventual
    consistency} model due to the distributed nature of the system. As a
    ...
    complete, use {{:  }ListKeyRotations} to view the details of the
    on-demand rotation.|}*)
...
type get_key_policy_request = {
  policy_name: string option;
   (**{|
    ...
    default value is [default]. The only valid name is [default]. To get the
...
type generate_random_response = {
  ciphertext_for_recipient: bytes option;
   (**{|
    ...
    Amazon Web Services Nitro Enclaves uses KMS} in the {i Key Management
...
type generate_random_request = {
  recipient: recipient_info option;
   (**{|
    ...
    decrypted only with the private key in the enclave. The [Plaintext] field
...
type generate_mac_request = {
  ...
  message: bytes;
   (**{|
    ...
    [GenerateMac] and {{:  }VerifyMac} do not provide special handling for
    message digests. If you generate an HMAC for a hash digest of a message,
    you must verify the HMAC of the same hash digest.|}*)
...
type generate_data_key_pair_request = {
  ...
  recipient: recipient_info option;
   (**{|
    ...
    Services Nitro Enclaves, use the
    {{: https://docs.aws.amazon.com/enclaves/latest/user/developing-applications.html#sdk }Amazon
    Web Services Nitro Enclaves SDK} to generate the attestation document and
    then use the Recipient parameter from any Amazon Web Services SDK to
    ...
    [CiphertextBlob] field in the response contains a copy of the private
    data key encrypted under the KMS key specified by the [KeyId] parameter.
    The [PrivateKeyPlaintext] field in the response is null or empty.
...
type generate_data_key_request = {
  ...
  recipient: recipient_info option;
   (**{|
    ...
    field in the response contains a copy of the data key encrypted under the
    KMS key specified by the [KeyId] parameter. The [Plaintext] field in the
  ...
  number_of_bytes: int option;
   (**{|
    ...
    (16-byte) and 256-bit (32-byte) data keys, use the [KeySpec] parameter.
...
type encrypt_request = {
  ...
  encryption_context: (string * string) list option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#cryptographic-operations }cryptographic
    operations} with a symmetric encryption KMS key. The standard asymmetric
...
type enable_key_rotation_request = {
  rotation_period_in_days: int option;
   (**{|
    ...
    You can use the
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/conditions-kms.html#conditions-kms-rotation-period-in-days }[kms:RotationPeriodInDays]}
    condition key to further constrain the values that principals can specify
...
type describe_key_request = {
  ...
  key_id: string;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html##aws-managed-cmk }Amazon
    Web Services managed key} and returns its [KeyId] and [Arn] in the
    response.
...
type custom_key_stores_list_entry = {
  ...
  connection_error_code: connection_error_code_type option;
   (**{|
    ...
    ({{:  }ConnectCustomKeyStore}). For additional help resolving these
    errors, see
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-failed }How
    to Fix a Connection Failure} in {i Key Management Service Developer
    Guide}.
    
    {b All custom key stores:}
    
    {ol 
          {- [INTERNAL_ERROR] — KMS could not complete the request due to
             an internal error. Retry the request. For
             [ConnectCustomKeyStore] requests, disconnect the custom key
    ...
    {ol 
          {- [CLUSTER_NOT_FOUND] — KMS cannot find the CloudHSM cluster
             with the specified cluster ID.
             }
          
          {- [INSUFFICIENT_CLOUDHSM_HSMS] — The associated CloudHSM cluster
             does not contain any active HSMs. To connect a custom key store
             to its CloudHSM cluster, the cluster must contain at least one
             active HSM.
             }
          ...
          {- [INSUFFICIENT_FREE_ADDRESSES_IN_SUBNET] — At least one private
             ...
             although two are preferable. For details, see
             {{: https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-failed }How
             to Fix a Connection Failure} in the {i Key Management Service
             Developer Guide}.
             }
          
          {- [INVALID_CREDENTIALS] — The [KeyStorePassword] for the custom
             ...
             [kmsuser] account password and update the [KeyStorePassword]
          ...
          {- [SUBNET_NOT_FOUND] — A subnet in the CloudHSM cluster
             ...
             key store. (This process creates a new cluster configuration
             with a VPC and private subnets.) For details, see
             {{: https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-failed }How
             to Fix a Connection Failure} in the {i Key Management Service
             Developer Guide}.
             }
          ...
          {- [USER_LOGGED_IN] — The [kmsuser] CU account is logged into the
             ...
             you can connect your custom key store to its CloudHSM cluster,
             you must log the [kmsuser] CU out of the cluster. If you changed
             the [kmsuser] password to log into the cluster, you must also
             and update the key store password value for the custom key
             store. For help, see
             {{: https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#login-kmsuser-2 }How
             to Log Out and Reconnect} in the {i Key Management Service
             Developer Guide}.
             }
          
          {- [USER_NOT_FOUND] — KMS cannot find a [kmsuser] CU account in
             the associated CloudHSM cluster. Before you can connect your
    ...
    {ol 
          ...
          {- [XKS_PROXY_ACCESS_DENIED] — KMS requests are denied access to
             the external key store proxy. If the external key store proxy
             has authorization rules, verify that they permit KMS to
          ...
          {- [XKS_PROXY_INVALID_RESPONSE] — KMS cannot interpret the
             response from the external key store proxy. If you see this
             connection error code repeatedly, notify your external key store
             proxy vendor.
             }
          
          {- [XKS_PROXY_INVALID_TLS_CONFIGURATION] — KMS cannot connect to
             the external key store proxy because the TLS configuration is
             invalid. Verify that the XKS proxy supports TLS 1.2 or 1.3.
             Also, verify that the TLS certificate is not expired, and that
             it matches the hostname in the [XksProxyUriEndpoint] value, and
             that it is signed by a certificate authority included in the
          ...
          {- [XKS_PROXY_NOT_REACHABLE] — KMS can't communicate with your
             ...
             and [XksProxyUriPath] are correct. Use the tools for your
             external key store proxy to verify that the proxy is active and
             available on its network. Also, verify that your external key
             manager instances are operating properly. Connection attempts
             fail with this connection error code if the proxy reports that
             all external key manager instances are unavailable.
             }
          
          {- [XKS_PROXY_TIMED_OUT] — KMS can connect to the external key
             store proxy, but the proxy does not respond to KMS in the time
             allotted. If you see this connection error code repeatedly,
             notify your external key store proxy vendor.
             }
          
          {- [XKS_VPC_ENDPOINT_SERVICE_INVALID_CONFIGURATION] — The Amazon
          ...
          {ol 
                ...
                {- The [Allow principals] list must include the KMS service
                   principal for the Region, [cks.kms..amazonaws.com], such
                ...
                {- It must have a private DNS name. The private DNS name for
                   an external key store with [VPC_ENDPOINT_SERVICE]
                   connectivity must be unique in its Amazon Web Services
                   Region.
                   }
                ...
                {- The
                   ...
                   certificate} specifies the private DNS hostname at which
          ...
          {- [XKS_VPC_ENDPOINT_SERVICE_NOT_FOUND] — KMS can't find the VPC
             endpoint service that it uses to communicate with the external
             key store proxy. Verify that the
             [XksProxyVpcEndpointServiceName] is correct and the KMS service
             principal has service consumer permissions on the Amazon VPC
             endpoint service.
             }
  ...
  connection_state: connection_state_type option;
   (**{|
    ...
    store. For an CloudHSM key store, the [ConnectionState] indicates whether
    it is connected to its CloudHSM cluster. For an external key store, the
    [ConnectionState] indicates whether it is connected to the external key
    ...
    never been connected or you use the {{:  }DisconnectCustomKeyStore}
    operation to disconnect it. If the value is [CONNECTED] but you are
    having trouble using the custom key store, make sure that the backing key
    store is reachable and active. For an CloudHSM key store, verify that its
  ...
  cloud_hsm_cluster_id: string option;
   (**{|
    ...
    CloudHSM key store. This field appears only when the [CustomKeyStoreType]
    is [AWS_CLOUDHSM].|}*)
...
type describe_custom_key_stores_request = {
  ...
  custom_key_store_name: string option;
   (**{|
    ...
    store, provide either the [CustomKeyStoreId] or [CustomKeyStoreName]
...
type derive_shared_secret_response = {
  key_origin: origin_type option;
   (**{|
    ...
    value is [EXTERNAL], the key material was imported or the KMS key doesn't
    have any key material.
...
type derive_shared_secret_request = {
  recipient: recipient_info option;
   (**{|
    ...
    the [CiphertextForRecipient] field in the response. This ciphertext can
    be decrypted only with the private key in the enclave. The
    [CiphertextBlob] field in the response contains the encrypted shared
  ...
  public_key: bytes;
   (**{|
    ...
    {{:  }GetPublicKey} returns the public key of an asymmetric KMS key pair
    in the required DER-encoded format.
    
    If you use
    {{: https://docs.aws.amazon.com/cli/v1/userguide/cli-chap-welcome.html }Amazon
    Web Services CLI version 1}, you must provide the DER-encoded X.509
  ...
  key_id: string;
   (**{|
    ...
    shared secret. The key usage of the KMS key must be [KEY_AGREEMENT]. To
    find the [KeyUsage] of a KMS key, use the {{:  }DescribeKey} operation.
...
(**{|
    ...
    {{:  }ScheduleKeyDeletion} operation to delete the KMS keys. After they
...
type delete_alias_request = {
  alias_name: string;
   (**{|
    The alias to be deleted. The alias name must begin with [alias/] followed
...
type decrypt_request = {
  ...
  encryption_algorithm: encryption_algorithm_spec option;
   (**{|
    ...
    This parameter is required only when the ciphertext was encrypted under
    an asymmetric KMS key. The default value, [SYMMETRIC_DEFAULT], represents
    the only supported algorithm that is valid for symmetric encryption KMS
    keys.|}*)
  ...
  key_id: string option;
   (**{|
    ...
    Enter a key ID of the KMS key that was used to encrypt the ciphertext. If
    you identify a different KMS key, the [Decrypt] operation throws an
    [IncorrectKeyException].
...
type create_key_request = {
  xks_key_id: string option;
   (**{|
    ...
    key store proxy} uses to refer to the external key. For help, see the
    ...
    {{: https://docs.aws.amazon.com/create-xks-keys.html#xks-key-requirements }Requirements
    for a KMS key in an external key store} in the {i Key Management Service
    Developer Guide}.
    ...
    the specified external key, a process known as {i double encryption}. For
    details, see
  ...
  multi_region: bool option;
   (**{|
    ...
    KMS key, omit this parameter or set it to [False]. The default value is
    [False].
  ...
  custom_key_store_id: string option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html }custom
    key store}. The [ConnectionState] of the custom key store must be
    [CONNECTED]. To find the CustomKeyStoreID and ConnectionState use the
    {{:  }DescribeCustomKeyStores} operation.
  ...
  origin: origin_type option;
   (**{|
    ...
    value to [EXTERNAL]. For more information about importing key material
    into KMS, see
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html }Importing
    Key Material} in the {i Key Management Service Developer Guide}. The
    ...
    associated CloudHSM cluster, set this value to [AWS_CLOUDHSM]. You must
    also use the [CustomKeyStoreId] parameter to identify the CloudHSM key
    store. The [KeySpec] value must be [SYMMETRIC_DEFAULT].
    
    To
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/create-xks-keys.html }create
    a KMS key in an external key store}, set this value to
    [EXTERNAL_KEY_STORE]. You must also use the [CustomKeyStoreId] parameter
  ...
  key_spec: key_spec option;
   (**{|
    ...
    used for encryption and decryption, except in China Regions, where it
    creates a 128-bit symmetric key that uses SM4 encryption. For help
    choosing a key spec for your KMS key, see
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/key-types.html#symm-asymm-choose }Choosing
    a KMS key type} in the {i {i Key Management Service Developer Guide}}.
    ...
    created. To further restrict the algorithms that can be used with the KMS
    key, use a condition key in its key policy or IAM policy. For more
    information, see
    ...
    or
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/policy-conditions.html#conditions-kms-signing-algorithm }kms:Signing
    Algorithm} in the {i {i Key Management Service Developer Guide}}.
    
    {{: http://aws.amazon.com/kms/features/#AWS_Service_Integration }Amazon
    Web Services services that are integrated with KMS} use symmetric
  ...
  key_usage: key_usage_type option;
   (**{|
    ...
    symmetric encryption KMS key; otherwise, it is required. You can't change
...
type create_grant_request = {
  ...
  name: string option;
   (**{|
    ...
    [GrantId] is returned. All grant tokens for the same grant ID can be used
    interchangeably.|}*)
  ...
  constraints: grant_constraints option;
   (**{|
    ...
    grant constraints can include the {{:  }DescribeKey} and
    ...
    [CreateGrant] permission have an equally strict or stricter encryption
    context constraint.
    ...
    grant constraints} in the {i Key Management Service Developer Guide}. For
    more information about encryption context, see
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#encrypt_context }Encryption
  ...
  retiring_principal: string option;
   (**{|
    ...
    {{:  }RevokeGrant} and
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/grant-manage.html#grant-delete }Retiring
    and revoking grants} in the {i Key Management Service Developer Guide}.|}*)
...
(**{|
    ...
    When you
    {{: https://docs.aws.amazon.com/cloudhsm/latest/userguide/initialize-cluster.html#sign-csr }initialize
    the CloudHSM cluster}, you create the trust anchor certificate and save
    it in the [customerCA.crt] file.|}*)
...
type create_custom_key_store_request = {
  xks_proxy_connectivity: xks_proxy_connectivity_type option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/plan-xks-keystore.html#choose-xks-connectivity }Choosing
  ...
  xks_proxy_authentication_credential: xks_proxy_authentication_credential_type option;
   (**{|
    ...
    credential, use the {{:  }UpdateCustomKeyStore} operation to provide the
   ...
   (**{|
    ...
    {ol 
          {- External key stores with [VPC_ENDPOINT_SERVICE] connectivity can
             share an Amazon VPC, but each external key store must have its
  ...
  xks_proxy_uri_endpoint: string option;
   (**{|
    ...
    ([.]), and hyphens ([-]). Additional slashes ([/] and [\]) are not
    permitted.
    ...
    {ol 
          ...
          {- An external key store with [PUBLIC_ENDPOINT] connectivity cannot
             use the same [XksProxyUriEndpoint] value as an external key
             store with [VPC_ENDPOINT_SERVICE] connectivity in this Amazon
             Web Services Region.
             }
  ...
  custom_key_store_type: custom_key_store_type option;
   (**{|
    ...
    or enter [AWS_CLOUDHSM]. For a custom key store backed by an external key
    manager outside of Amazon Web Services, enter [EXTERNAL_KEY_STORE]. You
  ...
  key_store_password: string option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/key-store-concepts.html#concept-kmsuser }[kmsuser]
    crypto user (CU) account} in the specified CloudHSM cluster. KMS logs
  ...
  cloud_hsm_cluster_id: string option;
   (**{|
    ...
    is required for custom key stores with [CustomKeyStoreType] of
    [AWS_CLOUDHSM].
...
type create_alias_request = {
  target_key_id: string;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/viewing-keys.html#find-cmk-id-arn }Finding
    the Key ID and ARN} in the {i {i Key Management Service Developer
    Guide}}.
  ...
  alias_name: string;
   (**{|
    ...
    a name, such as [alias/ExampleAlias].
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/kms/latest/developerguide/ }{i Key
    ...
    and install them, see {{: http://aws.amazon.com/tools/ }Tools for Amazon
    Web Services}.
    ...
    {{: https://docs.aws.amazon.com/general/latest/gr/kms.html#kms_region }Service
    endpoints} in the Key Management Service topic of the {i Amazon Web
    ...
    version. Clients must also support cipher suites with Perfect Forward
    ...
    {{: https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html }Signature
    Version 4}.
    ...
    made to KMS, who made the request, when it was made, and so on. To learn
    ...
    {ol 
          {- {{: https://docs.aws.amazon.com/general/latest/gr/aws-security-credentials.html }Amazon
             Web Services Security Credentials} - This topic provides general
             information about the types of credentials used to access Amazon
             Web Services.
             }
          
          {- {{: https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp.html }Temporary
             Security Credentials} - This section of the {i IAM User Guide}
             describes how to create and use temporary security credentials.
             }
          
          {- {{: https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html }Signature
             Version 4 Signing Process} - This set of topics walks you
