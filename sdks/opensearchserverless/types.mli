...
(**{|
    ...
    already exists or is being created. When deleting a resource, thrown when
    the resource is not in the ACTIVE or FAILED state.|}*)
...
type tag_resource_request = {
  ...
  resource_arn: string;
   (**{|
    ...
    active (not in the [DELETING] state), and must be owned by the account ID
...
type lifecycle_policy_error_detail = {
  ...
  error_message: string option;
   (**{|
    A description of the error. For example, [The specified Lifecycle Policy
    is not found].|}*)
...
type effective_lifecycle_policy_detail = {
  no_min_retention_period: bool option;
   (**{|
    ...
    that will return as [true] if the minimum number of days or hours is not
    set to a index resource.|}*)
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/opensearch-service/latest/developerguide/serverless-overview.html }What
...
type list_security_configs_request = {
  ...
  next_token: string option;
   (**{|
    ...
    you can include the returned [nextToken] in subsequent
    [ListSecurityConfigs] operations, which returns results in the next page.|}*)
...
type delete_collection_request = {
  ...
  id: string;
   (**{|
    ...
    ID is part of the collection endpoint. You can also retrieve it using the
    {{: https://docs.aws.amazon.com/opensearch-service/latest/ServerlessAPIReference/API_ListCollections.html }ListCollections}
    API.|}*)
