...
(**{|
    ...
    {ol 
          ...
          {- You obtain a shard iterator, but before you use the iterator in
             a [GetRecords] request, a stream record in the shard exceeds the
             24 hour period and is trimmed. This causes the iterator to
...
type stream_record = {
  ...
  approximate_creation_date_time: float option;
   (**{|
    ...
    {{: http://www.epochconverter.com/ }UNIX epoch time} format and rounded
...
type stream_description = {
  last_evaluated_shard_id: string option;
   (**{|
    ...
    reached the end of the result set is when [LastEvaluatedShardId] is
    empty.|}*)
  ...
  stream_view_type: stream_view_type option;
   (**{|
    ...
    {ol 
          ...
          {- [NEW_IMAGE] - entire items from the table, as they appeared
   ...
   (**{|
    ...
    {ol 
          ...
          {- [LATEST] - Start reading just after the most recent stream
             record in the shard, so that you always read the most recent
             data in the shard.
             }
...
(**{|
    ...
    stream records. A shard iterator expires 15 minutes after it is retrieved
    using the [GetShardIterator] action.|}*)
...
(**{|
    ...
    with Streams, see
    {{: https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Streams.html }Capturing
