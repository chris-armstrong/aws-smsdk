...
(**{|
    ...
    on-demand capacity mode. This API is only supported for data streams with
...
type update_shard_count_input = {
  ...
  target_shard_count: int;
   (**{|
    ...
    {ol 
          {- Set this value to more than double your current shard count for
             a stream.
             }
          ...
          {- Set this value to more than 10000 shards in a stream (the
             default limit for shard count per stream is 10000 per account
             per region), unless you request a limit increase.
             }
...
(**{|
    ...
    Key State Affects Use of a Customer Master Key} in the {i Amazon Web
...
type starting_position = {
  timestamp_: float option;
   (**{|
    ...
    a time stamp, set [StartingPosition] to [Type AT_TIMESTAMP]. A time stamp
    ...
    this exact time stamp does not exist, records will be streamed from the
...
type enhanced_metrics = {
  shard_level_metrics: metrics_name list option;
   (**{|
    ...
    the Amazon Kinesis Data Streams Service with Amazon CloudWatch} in the
...
type shard_filter = {
  ...
  type_: shard_filter_type;
   (**{|
    ...
    {ol 
          {- [AFTER_SHARD_ID] - the response includes all the shards,
             starting with the shard whose ID immediately follows the
             [ShardId] that you provided.
             }
          
          {- [AT_TRIM_HORIZON] - the response includes all the shards that
          ...
          {- [AT_LATEST] - the response includes only the currently open
...
type consumer = {
  ...
  consumer_ar_n: string;
   (**{|
    ...
    won't have the same ARN. That's because consumer ARNs contain the
...
type register_stream_consumer_output = {
  consumer: consumer;
   (**{|
    An object that represents the details of the consumer you registered.
    When you register a consumer, it gets an ARN that is generated by Kinesis
    Data Streams.|}*)
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/kinesis/latest/dev/service-sizes-and-limits.html }Streams
    Limits} in the {i Amazon Kinesis Data Streams Developer Guide}, and
    {{: https://docs.aws.amazon.com/general/latest/gr/api-retries.html }Error
    Retries and Exponential Backoff in Amazon Web Services} in the {i Amazon
...
type put_record_input = {
  ...
  sequence_number_for_ordering: string option;
   (**{|
    ...
    [SequenceNumberForOrdering] of record {i n} to the sequence number of
...
type list_stream_consumers_input = {
  ...
  next_token: string option;
   (**{|
    ...
    you specify [NextToken]. If you specify a [MaxResults] value that is less
    than the number of consumers that the operation returns if you don't
...
type list_shards_output = {
  next_token: string option;
   (**{|
    ...
    value for [MaxResults] that is less than the number of shards in the data
    stream, the response includes a pagination token named [NextToken]. You
    can specify this [NextToken] value in a subsequent call to [ListShards]
    to list the next set of shards. For more information about the use of
    this pagination token when calling the [ListShards] operation, see
...
type list_shards_input = {
  ...
  shard_filter: shard_filter option;
   (**{|
    ...
    the [AT_TRIM_HORIZON], [FROM_TRIM_HORIZON], or [AT_LATEST] types, you do
    not need to specify either the [ShardId] or the [Timestamp] optional
    properties.
    
    If you specify the [AFTER_SHARD_ID] type, you must also provide the value
    for the optional [ShardId] property. The [ShardId] property is identical
    in fuctionality to the [ExclusiveStartShardId] parameter of the
    [ListShards] API. When [ShardId] property is specified, the response
  ...
  next_token: string option;
   (**{|
    ...
    You can optionally specify a value for the [MaxResults] parameter when
    you specify [NextToken]. If you specify a [MaxResults] value that is less
    than the number of shards that the operation returns if you don't specify
...
type get_shard_iterator_input = {
  ...
  shard_iterator_type: shard_iterator_type;
   (**{|
    ...
    {ol 
          {- AT_SEQUENCE_NUMBER - Start reading from the position denoted by
...
type create_stream_input = {
  stream_mode_details: stream_mode_details option;
   (**{|
    ...
    Data Streams, you can choose between an {b on-demand} capacity mode and a
    {b provisioned} capacity mode for your data streams.|}*)
  ...
  stream_name: string;
   (**{|
    ...
    is also scoped by Amazon Web Services Region. That is, two streams in two
    different Amazon Web Services accounts can have the same name. Two
