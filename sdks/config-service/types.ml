...
(**{|
    ...
    required fields or if the input value fails the validation, or if you are
    trying to create more than 300 queries.
    
    For GetStoredQuery, ListStoredQuery, and DeleteStoredQuery you will see
    this exception if there are missing required fields or if the input value
...
(**{|
    ...
    conformance pack using an SSM document, first create an SSM document with
...
type status_detail_filters = {
  member_account_rule_status: member_account_rule_status option;
   (**{|
    ...
    time, Config rule status is created in the member account. When
    ...
    {ol 
          ...
          {- [UPDATE_FAILED] when Config rule deletion has failed in the
             member account.
             }
...
type resource_details = {
  resource_configuration_schema_type: resource_configuration_schema_type option;
   (**{|
    ...
    type schema}, or [CFN_RESOURCE_SCHEMA], in "{i Amazon Web Services public
    extensions}" within the CloudFormation registry or with the following CLI
    commmand: [aws cloudformation describe-type --type-name "AWS::S3::Bucket"
    --type RESOURCE].
    ...
    Web Services resource and property types reference} in the CloudFormation
    User Guide.
    |}*)
...
(**{|
    ...
    {ol 
          ...
          {- For PutOrganizationConfigRule, organization Config rule cannot
             be created because you do not have permissions to call IAM
          ...
          {ol 
                ...
                {- You do not have permission to read Amazon S3 bucket or
                   call SSM:GetDocument.
                   }
...
(**{|
    ...
    {ol 
          {- For DeleteConfigRule, Config is deleting this rule. Try your
             request again later.
             }
          
          {- For DeleteConfigRule, the rule is deleting your evaluation
             results. Try your request again later.
             }
          
          {- For DeleteConfigRule, a remediation action is associated with
             the rule and Config cannot delete this rule. Delete the
             remediation action associated with the rule before deleting the
             rule and try your request again later.
             }
          
          {- For PutConfigOrganizationRule, organization Config rule deletion
             is in progress. Try your request again later.
             }
          
          {- For DeleteOrganizationConfigRule, organization Config rule
             creation is in progress. Try your request again later.
             }
          
          {- For PutConformancePack and PutOrganizationConformancePack, a
             conformance pack creation, update, and deletion is in progress.
...
type put_resource_config_request = {
  ...
  resource_type: string;
   (**{|
    ...
    You cannot use the organization names “amzn”, “amazon”,
    “alexa”, “custom” with custom resource types. It is the first
    part of the ResourceType up to the first ::.
    |}*)
...
type remediation_configuration = {
  ...
  retry_attempt_seconds: int option;
   (**{|
    ...
    remediation attempts have been made under [RetryAttemptSeconds], a
    remediation exception will be added to the resource. If you do not select
  ...
  maximum_automatic_attempts: int option;
   (**{|
    ...
    RetryAttemptSeconds as 50 seconds, Config will put a RemediationException
    on your behalf for the failing resource after the 5th failed attempt
    within 50 seconds.|}*)
  ...
  target_version: string option;
   (**{|
    ...
    call PutRemediationConfiguration API again to ensure the remediations can
    run.
    |}*)
...
(**{|
    ...
    {ol 
          ...
          {- The configuration aggregator cannot be updated because your
             Amazon Web Services Organization management account or the
             delegated administrator role changed. Delete this aggregator and
             create a new one with the current Amazon Web Services
             Organization.
             }
          
          {- The configuration aggregator is associated with a previous
             Amazon Web Services Organization and Config cannot aggregate
             data with current Amazon Web Services Organization. Delete this
             aggregator and create a new one with the current Amazon Web
             Services Organization.
             }
          
          {- You are not a registered delegated administrator for Config with
             permissions to call [ListDelegatedAdministrators] API. Ensure
             that the management account registers delagated administrator
             for Config service principle name before the delegated
   ...
   (**{|
    ...
    {ol 
          ...
          {- [OversizedConfigurationItemChangeNotification] - Triggers an
             ...
             Config may generate this notification type when a resource
...
(**{|
    ...
    accounts have debug logging enabled, and other custom rule metadata, such
    as resource type, resource ID of Amazon Web Services resource, and
...
type organization_custom_policy_rule_metadata = {
  ...
  policy_runtime: string;
   (**{|
    ...
    Guard is a policy-as-code language that allows you to write policies that
    are enforced by Config Custom Policy rules. For more information about
    Guard, see the
...
type put_organization_config_rule_request = {
  ...
  organization_managed_rule_metadata: organization_managed_rule_metadata option;
   (**{|
    ...
    Web Services resource along with the rule identifier. It also provides
    the frequency with which you want Config to run evaluations for the rule
...
(**{|
    ...
    {ol 
          {- The value for the [deliveryFrequency] parameter within the
             delivery channel configuration, which sets how often Config
             delivers configuration snapshots. This value also sets how often
             Config invokes evaluations for Config rules.
             }
          
          {- The value for the [MaximumExecutionFrequency] parameter, which
             sets the maximum frequency with which Config invokes evaluations
             for the rule. For more information, see {{:  }ConfigRule}.
             }
    ...
    {ol 
          {- For example, you want your rule to run evaluations when Config
             delivers the configuration snapshot.
             }
          
          {- You specify the [MaximumExecutionFrequency] value for
             [Six_Hours].
             }
          
          {- You then specify the delivery channel [deliveryFrequency] value
...
type delivery_channel = {
  ...
  s3_kms_key_arn: string option;
   (**{|
    ...
    key (KMS key) used to encrypt objects delivered by Config. Must belong to
...
(**{|
    You have reached the limit of the number of configuration recorders you
    can create.|}*)
    ...
    {ol 
          ...
          {ol 
                {- Setting the [allSupported] field of
                   ...
                   to [true], but providing a non-empty list for the
          ...
          {- You have reached the limit of the number of resource types you
             can provide for the recording group.
             }
          
          {- You have provided resource types or a recording strategy that
             are not valid.
             }
...
(**{|
    ...
    be recorded automatically in all supported Config Regions were the
    configuration recorder is enabled.
    
    IAM users, groups, roles, and customer managed policies will be recorded
    in the Region where you set up the configuration recorder if that is a
    Region where Config was available before February 2022. You cannot be
    record the global IAM resouce types in Regions supported by Config after
    February 2022. This list where you cannot record the global IAM resource
   ...
   (**{|
    ...
    {ol 
          {- If you set this option to [ALL_SUPPORTED_RESOURCE_TYPES], Config
             ...
             {{: https://docs.aws.amazon.com/config/latest/APIReference/API_RecordingGroup.html }RecordingGroup}
             to [true]. When Config adds support for a new resource type,
          ...
          {- If you set this option to [INCLUSION_BY_RESOURCE_TYPES], Config
             records configuration changes for only the resource types that
             you specify in the [resourceTypes] field of
             {{: https://docs.aws.amazon.com/config/latest/APIReference/API_RecordingGroup.html }RecordingGroup}.
    ...
    The [recordingStrategy] field is optional when you set the [allSupported]
    field of
    {{: https://docs.aws.amazon.com/config/latest/APIReference/API_RecordingGroup.html }RecordingGroup}
    to [true].
...
(**{|
    ...
    By default, Config records configuration changes for all current and
    future supported resource types in the Amazon Web Services Region where
    you have enabled Config, excluding the global IAM resource types: IAM
    ...
    exclude specific resources types. For a list of supported resource types,
    see
    {{: https://docs.aws.amazon.com/config/latest/developerguide/resource-config-reference.html#supported-resources }Supported
    Resource Types} in the {i Config developer guide}.
    
    If you don't want Config to record all current and future supported
    resource types (excluding the global IAM resource types), use one of the
    ...
    {ol 
          ...
          {- {b Record specific resource types}
             ([INCLUSION_BY_RESOURCE_TYPES]).
             }
    ...
    resource types} ([ALL_SUPPORTED_RESOURCE_TYPES]), you can use the flag
    [includeGlobalResourceTypes] to include the global IAM resource types in
    your recording.
...
type recording_group = {
   ...
   (**{|
    ...
    {ol 
          {- If you set the [useOnly] field of
             ...
             changes for all supported resource types, excluding the global
             IAM resource types. You also must set the [allSupported] field
             of
             {{: https://docs.aws.amazon.com/config/latest/APIReference/API_RecordingGroup.html }RecordingGroup}
             to [true]. When Config adds support for a new resource type,
             Config automatically starts recording resources of that type.
          ...
          {- If you set the [useOnly] field of
             ...
             changes for all supported resource types except the resource
  ...
  resource_types: resource_type list option;
   (**{|
    ...
    For a list of valid [resourceTypes] values, see the {b Resource Type
    Value} column in
    {{: https://docs.aws.amazon.com/config/latest/developerguide/resource-config-reference.html#supported-resources }Supported
    ...
    {{: https://docs.aws.amazon.com/config/latest/APIReference/API_RecordingGroup.html }RecordingGroup}
    to [true], and either omit this field or don't specify any resource types
    in this field. If you set the [allSupported] field to [false] and specify
    values for [resourceTypes], when Config adds support for a new type of
    resource, it will not record resources of that type unless you manually
    ...
    Coverage by Region Availability} to see if the resource type is supported
    ...
    recording of that resource type in all Regions supported by Config, even
    if the specified resource type is not supported in the Amazon Web
  ...
  include_global_resource_types: bool option;
   (**{|
    ...
    global IAM resource types can only be recorded by Config in Regions where
    Config was available before February 2022. You cannot be record the
    global IAM resouce types in Regions supported by Config after February
    2022. This list where you cannot record the global IAM resource types
    ...
    The [AWS::RDS::GlobalCluster] resource type will be recorded in all
    supported Config Regions where the configuration recorder is enabled,
    even if [includeGlobalResourceTypes] is set[false]. The
    [includeGlobalResourceTypes] option is a bundle which only applies to IAM
    users, groups, roles, and customer managed policies.
    ...
    [EXCLUSION_BY_RESOURCE_TYPES] recording strategy. This means that the
    global IAM resource types (IAM users, groups, roles, and customer managed
    policies) will not be automatically added as exclusions for
    [exclusionByResourceTypes] when [includeGlobalResourceTypes] is set to
    [false].
    
    The [includeGlobalResourceTypes] field should only be used to modify the
    [AllSupported] field, as the default for the [AllSupported] field is to
    record configuration changes for all supported resource types excluding
    the global IAM resource types. To include the global IAM resource types
    when [AllSupported] is set to [true], make sure to set
    [includeGlobalResourceTypes] to [true].
    
    To exclude the global IAM resource types for the
    [EXCLUSION_BY_RESOURCE_TYPES] recording strategy, you need to manually
    add them to the [resourceTypes] field of [exclusionByResourceTypes].
    ...
    make sure to not list them in the [resourceTypes] field in addition to
    setting the [includeGlobalResourceTypes] field to false.
    |}*)
  ...
  all_supported: bool option;
   (**{|
    ...
    Check
    {{: https://docs.aws.amazon.com/config/latest/developerguide/what-is-resource-config-coverage.html }Resource
    Coverage by Region Availability} to see if a resource type is supported
   ...
   (**{|
    ...
    {ol 
          ...
          {- Daily recording allows you to receive a configuration item (CI)
             representing the most recent state of your resources over the
             last 24-hour period, only if it’s different from the previous
             CI recorded.
             }
    ...
    resources. If you are using Firewall Manager, it is recommended that you
...
type recording_mode = {
  recording_mode_overrides: recording_mode_override list option;
   (**{|
    ...
    overrides for the recording mode. The [recordingModeOverride] object in
    the [recordingModeOverrides] array consists of three fields: a
  ...
  recording_frequency: recording_frequency;
   (**{|
    ...
    For the {b allSupported} ([ALL_SUPPORTED_RESOURCE_TYPES]) recording
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/config/latest/developerguide/stop-start-recorder.html }{b Managing
    the Configuration Recorder}} in the {i Config Developer Guide}.|}*)
type configuration_recorder = {
  ...
  recording_group: recording_group option;
   (**{|
    ...
    and deleting these temporary resources. An {i ephemeral workload} is a
    temporary use of computing resources that are loaded and run when needed.
    Examples include Amazon Elastic Compute Cloud (Amazon EC2) Spot
  ...
  role_ar_n: string option;
   (**{|
    ...
    Security Hub or Control Tower, and an Config role has already been
    ...
    {{: https://docs.aws.amazon.com/config/latest/developerguide/security-iam.html }{b Identity
    and Access Management for Config}} in the {i Config Developer Guide}.
    |}*)
...
type scope = {
  ...
  compliance_resource_types: string list option;
   (**{|
    ...
    want to trigger an evaluation for the rule. You can only specify one type
    if you also specify a resource ID for [ComplianceResourceId].|}*)
...
(**{|
    ...
    enabled. You can specify the following CustomPolicyDetails parameter
    values only for Config Custom Policy rules.|}*)
...
(**{|
    ...
    change to an Amazon Web Services resource or at a periodic frequency that
    you choose (for example, every 24 hours). There are two types of rules:
    ...
    ({{: https://docs.aws.amazon.com/config/latest/developerguide/gettingstarted-concepts.html#gettingstarted-concepts-function }Lambda
    Developer Guide}) and with Guard
    ({{: https://github.com/aws-cloudformation/cloudformation-guard }Guard
    GitHub Repository}), a policy-as-code language. Config custom rules
    created with Lambda are called {i Config Custom Lambda Rules} and Config
...
type config_rule = {
  ...
  config_rule_state: config_rule_state option;
   (**{|
    ...
    by Config. It can also indicate the evaluation status for the Config
    rule.
    ...
    Config sets the state of the rule to [DELETING_RESULTS] temporarily after
    you use the [DeleteEvaluationResults] request to delete the current
  ...
  maximum_execution_frequency: maximum_execution_frequency option;
   (**{|
    ...
    {ol 
          {- This is for an Config managed rule that is triggered at a
             periodic frequency.
             }
...
type list_tags_for_resource_request = {
  ...
  limit: int option;
   (**{|
    ...
    50. You cannot specify a number greater than 50. If you specify 0, Config
    uses the default.|}*)
...
type conformance_pack_compliance_scores_filters = {
  conformance_pack_names: string list;
   (**{|
    ...
    include up to 25 conformance packs in the [ConformancePackNames] array of
    strings, each with a character limit of 256 characters for the
...
type list_conformance_pack_compliance_scores_request = {
  ...
  sort_by: sort_by option;
   (**{|
    ...
    order by name of the conformance pack. Enter [SCORE], to sort conformance
    pack compliance scores by the numerical value of the compliance score.|}*)
...
type configuration_item = {
  configuration_item_delivery_time: float option;
   (**{|
    ...
    field will be omitted since the delivery time is instantaneous as the CI
    is available right away. For more information on daily recording and
    continuous recording, see
  ...
  related_events: string list option;
   (**{|
    ...
    by any event. As of Version 1.3, the relatedEvents field is empty. You
    can access the
    {{: https://docs.aws.amazon.com/awscloudtrail/latest/APIReference/API_LookupEvents.html }LookupEvents
    API} in the {i CloudTrail API Reference} to retrieve the events for the
    resource.|}*)
...
(**{|
    The Config rule in the request is not valid. Verify that the rule is an
    organization Config Process Check rule, that the rule name is correct,
...
type organization_conformance_pack_detailed_status = {
  ...
  status: organization_resource_detailed_status;
   (**{|
    ...
    account. Conformance pack status is deleted when the management account
    ...
    {ol 
          {- [CREATE_SUCCESSFUL] when conformance pack has been created in
             the member account.
             }
          ...
          {- [DELETE_IN_PROGRESS] when conformance pack is being deleted in
             the member account.
             }
          
          {- [DELETE_SUCCESSFUL] when conformance pack has been deleted in
             the member account.
             }
          
          {- [UPDATE_SUCCESSFUL] when conformance pack has been updated in
             the member account.
             }
          
          {- [UPDATE_IN_PROGRESS] when conformance pack is being updated in
             the member account.
             }
...
type get_discovered_resource_counts_response = {
  ...
  total_discovered_resources: int option;
   (**{|
    ...
    {ol 
          {- Config is recording three resource types in the US East (Ohio)
...
type get_discovered_resource_counts_request = {
  ...
  resource_types: string list option;
   (**{|
    ...
    of {{:  }ResourceCount} objects. If the configuration recorder is not
...
type get_aggregate_discovered_resource_counts_request = {
  ...
  limit: int option;
   (**{|
    The maximum number of {{:  }GroupedResourceCount} objects returned on
...
type get_aggregate_compliance_details_by_config_rule_request = {
  ...
  compliance_type: compliance_type option;
   (**{|
    ...
    Config supports only the [COMPLIANT] and [NON_COMPLIANT]. Config does not
    support the [NOT_APPLICABLE] and [INSUFFICIENT_DATA] values.
    |}*)
...
type organization_conformance_pack_status = {
  ...
  status: organization_resource_status;
   (**{|
    ...
    time, conformance pack status is created in all the member accounts. When
    management account calls PutOrganizationConformancePack for the second
    ...
    {ol 
          {- [CREATE_SUCCESSFUL] when an organization conformance pack has
          ...
          {- [DELETE_SUCCESSFUL] when an organization conformance pack has
             been successfully deleted from all the member accounts.
             }
          
          {- [UPDATE_SUCCESSFUL] when an organization conformance pack has
             been successfully updated in all the member accounts.
             }
          
          {- [UPDATE_IN_PROGRESS] when an organization conformance pack
  ...
  organization_rule_status: organization_rule_status;
   (**{|
    ...
    {ol 
          ...
          {- [CREATE_IN_PROGRESS] when an organization Config rule creation
             is in progress.
             }
          ...
          {- [DELETE_IN_PROGRESS] when an organization Config rule deletion
             is in progress.
             }
...
(**{|
    ...
    Web Services resource, and organization trigger types that trigger Config
  ...
  conformance_pack_state: conformance_pack_state;
   (**{|
    ...
    {ol 
          ...
          {- DELETE_IN_PROGRESS when a conformance pack deletion is in
             progress.
             }
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/config/latest/developerguide/evaluate-config-rules.html }{b Evaluation
...
(**{|
    ...
    rules. The status includes information such as the last time the rule
...
type config_rule_evaluation_status = {
  ...
  last_debug_log_delivery_status_reason: string option;
   (**{|
    ...
    last failed attempt to retrieve a debug log for your Config Custom Policy
    rules.|}*)
...
(**{|
    ...
    or a security group. For a complete list of resources currently supported
    by Config, see
    {{: https://docs.aws.amazon.com/config/latest/developerguide/resource-config-reference.html#supported-resources }Supported
    ...
    Version 4 Signing Process}. For detailed information about Config
    features and their associated actions or commands, as well as how to work
    with Amazon Web Services Management Console, see
    {{: https://docs.aws.amazon.com/config/latest/developerguide/WhatIsConfig.html }What
    Is Config} in the {i Config Developer Guide}.|}*)
