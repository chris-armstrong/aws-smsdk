...
(**{|
    ...
    {ol 
          {- {b InternalServiceException} – This can mean that the Resource
             Groups Tagging API didn't receive a response from another Amazon
             Web Services service. It can also mean that the resource type in
             the request is not supported by the Resource Groups Tagging API.
          ...
          {- {b AccessDeniedException} – This can mean that you need
             permission to call the tagging operations in the Amazon Web
             Services service that contains the resource. For example, to use
             the Resource Groups Tagging API to tag a Amazon CloudWatch alarm
...
type untag_resources_output = {
  failed_resources_map: (string * failure_info) list option;
   (**{|
    ...
    error message. If there are no errors, the [FailedResourcesMap] is empty.|}*)
...
(**{|
    ...
    {ol 
          ...
          {- You can't access the Amazon S3 bucket for report storage. For
             ...
             Requirements for Organization-wide Tag Compliance Reports} in
...
(**{|
    ...
    Amazon Web Services Resources} in the {i Amazon Web Services General
    Reference}.|}*)
type tag = {
  value: string;
   (**{|
...
type summary = {
  ...
  target_id: string option;
   (**{|
    ...
    don't know the root ID, you can call the Organizations
...
(**{|
    ...
    {ol 
          {- You must meet the prerequisites for using tag policies. For
             ...
             and Permissions for Using Tag Policies} in the {i Organizations
             User Guide.}
             }
          ...
          {- You must have a tag policy attached to the organization root, an
             OU, or an account.
             }
...
type get_tag_values_output = {
  ...
  pagination_token: string option;
   (**{|
    ...
    response value as the [PaginationToken] value in the request for the next
    page.|}*)
...
type get_resources_input = {
  ...
  tag_filters: tag_filter list option;
   (**{|
    ...
    {ol 
          {- If you {i don't} specify a [TagFilter], the response includes
             all resources that are currently tagged or ever had a tag.
             Resources that currently don't have tags are shown with an empty
          ...
          {- If you specify a filter that contains more than one value for a
             key, the response returns resources that match {i any} of the
          ...
          {ol 
                ...
                {- [GetResources({filter3})] returns resources tagged with
                   any tag with the key [key3], and with any or no value
                   }
                
                {- [GetResources({filter1,filter2,filter3})] returns
                   resources tagged with [(key1=value1) and (key2=value2 or
                   key2=value3 or key2=value4) and (key3, any or no value)]
                   }
...
type get_compliance_summary_input = {
  ...
  resource_type_filters: string list option;
   (**{|
    ...
    {i {{: https://docs.aws.amazon.com/general/latest/gr/ }Amazon Web
    Services General Reference}} for the following:
  ...
  status: string option;
   (**{|
    ...
    {ol 
          ...
          {- [SUCCEEDED] - Report creation is complete. You can open the
