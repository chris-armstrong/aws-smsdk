...
(**{|
    ...
    the definition return in the response as {b diagnostic elements}, rather
...
type validate_state_machine_definition_output = {
  diagnostics: validate_state_machine_definition_diagnostic list;
   (**{|
    ...
    this field will contain an array of {b Diagnostic} objects to help you
    troubleshoot.|}*)
...
type logging_configuration = {
  destinations: log_destination list option;
   (**{|
    ...
    will be logged. Limited to size 1. Required, if your log level is not set
    to [OFF].|}*)
...
(**{|
    ...
    specify up to two state machine versions. You also specify the percentage
    of traffic to be routed to each version.|}*)
...
(**{|
    Your [tracingConfiguration] key does not match, or [enabled] has not been
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/step-functions/latest/dg/limits.html }Limits
    Topic} in the Step Functions Developer Guide.|}*)
type too_many_tags = {
  resource_name: string option;
...
(**{|
    Contains additional details about the state's execution, including its
    input and output data processing flow, and HTTP response information. The
...
type test_state_output = {
  ...
  next_state: string option;
   (**{|
    ...
    this ﬁeld doesn't contain a value.|}*)
...
type test_state_input = {
  reveal_secrets: bool option;
   (**{|
    ...
    connection adds to modify the HTTP request headers, query parameters, and
    body. Step Functions doesn't omit any information included in the state
    definition or the HTTP response.
    
    If you set [revealSecrets] to [true], you must make sure that the IAM
    user that calls the [TestState] API has permission for the
    [states:RevealSecrets] action. For an example of IAM policy that sets the
    [states:RevealSecrets] permission, see
   ...
   (**{|
    ...
    {ol 
          ...
          {- [TRACE]: Shows the HTTP request and response for an HTTP Task.
             This level also shows the final state output along with the
             input and output data processing result.
             }
...
(**{|
    The task token has either expired or the task associated with the token
...
type task_succeeded_event_details = {
  ...
  output: string option;
   (**{|
    ...
    response becomes the output of the related task. Length constraints apply
...
type state_machine_alias_list_item = {
  ...
  state_machine_alias_arn: string;
   (**{|
    ...
    alias ARN is a combination of state machine ARN and the alias name
...
type start_execution_input = {
  ...
  name: string option;
   (**{|
    ...
    information, see
    {{: https://docs.aws.amazon.com/step-functions/latest/dg/limits.html#service-limits-state-machine-executions }Limits
    Related to State Machine Executions} in the {i Step Functions Developer
    Guide}.
  ...
  state_machine_arn: string;
   (**{|
    ...
    {ol 
          {- {b An unqualified state machine ARN} – Refers to a state
             machine ARN that isn't qualified with a version or alias ARN.
             The following is an example of an unqualified state machine ARN.
          ...
          {- {b A state machine version ARN} – Refers to a version ARN,
             which is a combination of state machine ARN and the version
             number separated by a colon (:). The following is an example of
             the ARN for version 10.
          ...
          {- {b A state machine alias ARN} – Refers to an alias ARN, which
             is a combination of state machine ARN and the alias name
             separated by a colon (:). The following is an example of the ARN
             for an alias named [PROD].
...
type send_task_success_input = {
  ...
  task_token: string;
   (**{|
    ...
    object} when a workflow enters a task state. See
...
type publish_state_machine_version_input = {
  ...
  revision_id: string option;
   (**{|
    ...
    assigned, specify the string [INITIAL] for the [revisionId] parameter.
    For example, you can specify a [revisionID] of [INITIAL] when you create
...
type map_run_item_counts = {
  ...
  failures_not_redrivable: int option;
   (**{|
    ...
    child workflows is terminal. For example, child workflows with an
    execution status of [FAILED], [ABORTED], or [TIMED_OUT] and a
    [redriveStatus] of [NOT_REDRIVABLE].|}*)
  ...
  aborted: int;
   (**{|
    ...
    were either stopped by the user or by Step Functions, because the Map Run
    failed.|}*)
...
type map_run_execution_counts = {
  pending_redrive: int option;
   (**{|
    ...
    be redriven. The status of these child workflow executions could be
    [FAILED], [ABORTED], or [TIMED_OUT] in the original execution attempt or
    a previous redrive attempt.|}*)
  ...
  results_written: int;
   (**{|
    ...
    by [ResultWriter]. For more information, see
    {{: https://docs.aws.amazon.com/step-functions/latest/dg/input-output-resultwriter.html }ResultWriter}
    in the {i Step Functions Developer Guide}.|}*)
...
type execution_list_item = {
  ...
  redrive_count: int option;
   (**{|
    ...
    redriven an execution, the [redriveCount] is 0. This count is only
...
type list_executions_input = {
  ...
  map_run_arn: string option;
   (**{|
    ...
    workflow executions. If the [mapRunArn] field is specified, a list of all
    of the child workflow executions started by a Map Run is returned. For
...
type get_activity_task_output = {
  ...
  task_token: string option;
   (**{|
    ...
    included in subsequent calls to {{:  }SendTaskHeartbeat},
    {{:  }SendTaskSuccess} or {{:  }SendTaskFailure} in order to report the
...
type describe_state_machine_output = {
  ...
  label: string option;
   (**{|
    ...
    This parameter is present only if the [stateMachineArn] specified in
    input is a qualified state machine ARN.|}*)
  ...
  state_machine_arn: string;
   (**{|
    ...
    machine ARN and the version number separated by a colon (:). For example,
    [stateMachineARN:1].|}*)
...
type describe_state_machine_input = {
  state_machine_arn: string;
   (**{|
    ...
    about that version. The version ARN is a combination of state machine ARN
    and the version number separated by a colon (:). For example,
    [stateMachineARN:1].|}*)
...
type describe_state_machine_for_execution_output = {
  ...
  label: string option;
   (**{|
    A user-defined or an auto-generated string that identifies a [Map] state.
    This ﬁeld is returned only if the [executionArn] is a child workflow
...
type describe_map_run_output = {
  ...
  redrive_count: int option;
   (**{|
    ...
    redriven a Map Run, the [redriveCount] is 0. This count is only updated
    if you successfully redrive a Map Run.|}*)
...
type describe_execution_output = {
   ...
   (**{|
    ...
    {ol 
          {- For executions of type [STANDARD], or for a Distributed Map that
             includes child workflows of type [STANDARD],
             [redriveStatusReason] can include one of the following reasons:
          ...
          {ol 
                ...
                {- [Execution was started before the launch of
                   RedriveExecution].
                   }
          ...
          {- For a Distributed Map that includes child workflows of type
             ...
             workflows are not redrivable. This happens when the child
             workflow executions have completed successfully.
             }
   ...
   (**{|
    ...
    {ol 
          {- For executions of type [STANDARD], [redriveStatus] is
             [NOT_REDRIVABLE] if calling the {{:  }RedriveExecution} API
             action would return the [ExecutionNotRedrivable] error.
             }
          ...
          You can redrive failed or timed out [EXPRESS] workflows {i only if}
          they're a part of a Map Run. When you
          {{: https://docs.aws.amazon.com/step-functions/latest/dg/redrive-map-run.html }redrive}
          the Map Run, these workflows are restarted using the
  ...
  state_machine_alias_arn: string option;
   (**{|
    ...
    the execution. The alias ARN is a combination of state machine ARN and
    the alias name separated by a colon (:). For example,
  ...
  state_machine_version_arn: string option;
   (**{|
    The Amazon Resource Name (ARN) of the state machine version associated
    with the execution. The version ARN is a combination of state machine ARN
    and the version number separated by a colon (:). For example,
    [stateMachineARN:1].
...
(**{|
    ...
    Web Services. You can access and use Step Functions using the console,
    the Amazon Web Services SDKs, or an HTTP API. For more information about
    Step Functions, see the
    {i {{: https://docs.aws.amazon.com/step-functions/latest/dg/welcome.html }Step
