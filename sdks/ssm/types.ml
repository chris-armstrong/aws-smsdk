...
(**{|
    ...
    A sync source of this type can synchronize data from Organizations or, if
    an Amazon Web Services organization isn't present, from multiple Amazon
...
type resource_data_sync_aws_organizations_source = {
  ...
  organization_source_type: string;
   (**{|
    ...
    [OrganizationalUnits] or [EntireOrganization]. For [OrganizationalUnits],
    the data is aggregated from a set of organization units. For
    [EntireOrganization], the data is aggregated from the entire Amazon Web
...
type resource_data_sync_source = {
  enable_all_ops_data_sources: bool option;
   (**{|
    ...
    Regions} in the {i Amazon Web Services Systems Manager User Guide}.|}*)
...
(**{|
    ...
    [PATCH_SET], [PRODUCT], [PRODUCT_FAMILY], [CLASSIFICATION], and
    [MSRC_SEVERITY].
    ...
    accepts all patches where product name is either "Office 2013" or "Office
    2016". The filter values can be exact values for the patch property given
    as a key, or a wildcard (*), which matches all values.
...
type update_patch_baseline_request = {
  ...
  rejected_patches_action: patch_action option;
   (**{|
    ...
    {ol 
          {- {b [ALLOW_AS_DEPENDENCY]}: A package in the [Rejected] patches
             ...
             status is reported as [InstalledOther]. This is the default
             action if no option is specified.
             }
          
          {- {b BLOCK}: Packages in the {b Rejected patches} list, and
             packages that include them as dependencies, aren't installed by
             Patch Manager under any circumstances. If a package was
             installed before it was added to the {b Rejected patches} list,
             or is installed outside of Patch Manager afterward, it's
             considered noncompliant with the patch baseline and its status
             is reported as {i InstalledRejected}.
             }
  ...
  rejected_patches: string list option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/patch-manager-approved-rejected-package-name-formats.html }About
    package name formats for approved and rejected patch lists} in the
    {i Amazon Web Services Systems Manager User Guide}.|}*)
...
(**{|
    ...
    Manager, see
    {{: https://docs.aws.amazon.com/general/latest/gr/ssm.html#limits_ssm }Systems
    Manager service quotas} in the {i Amazon Web Services General Reference}.|}*)
...
(**{|
    ...
    example, related OpsItems can include OpsItems with similar error
...
type update_ops_item_request = {
  ...
  status: ops_item_status option;
   (**{|
    ...
    OpsItem details} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
  ...
  operational_data: (string * ops_item_data_value) list option;
   (**{|
    ...
    Operational data keys {i can't} begin with the following: [amazon],
    ...
    {{:  }DescribeOpsItems} API operation) can view and search on the
    specified data. Operational data that isn't searchable is only viewable
    by users who have access to the OpsItem (as provided by the
    ...
    OpsItems manually} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
...
type update_managed_instance_role_request = {
  iam_role: string;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-service-role.html }Create
    an IAM service role for a hybrid and multicloud environment} in the
    {i Amazon Web Services Systems Manager User Guide}.
...
(**{|
    ...
    {ol 
          ...
          {- SSM Agent isn't registered with the SSM endpoint. Try
             reinstalling SSM Agent.
             }
          
          {- The managed node isn't in a valid state. Valid states are:
             [Running], [Pending], [Stopped], and [Stopping]. Invalid states
...
(**{|
    ...
    maintenance window tasks without targets} in the {i Amazon Web Services
    ...
    {ol 
          ...
          {- Additionally, Maintenance Windows support targeting resource
             types:
    ...
    multiple managed nodes} in the {i Amazon Web Services Systems Manager
    User Guide}.|}*)
type target = {
...
type alarm_configuration = {
  ...
  ignore_poll_alarm_failure: bool option;
   (**{|
    ...
    CloudWatch. In cases where we successfully retrieve an alarm status of OK
    or INSUFFICIENT_DATA, the automation or command continues to run,
...
type update_maintenance_window_task_request = {
  ...
  cutoff_behavior: maintenance_window_task_cutoff_behavior option;
   (**{|
    ...
    {ol 
          ...
          {ol 
                {- For Automation, Lambda, Step Functions tasks: When the
                   cutoff time is reached, any task invocations that are
                   already running continue, but no new task invocations are
                   started.
                   }
                
                {- For Run Command tasks: When the cutoff time is reached,
                   the system sends a {{:  }CancelCommand} operation that
                   attempts to cancel the command associated with the task.
  ...
  max_errors: string option;
   (**{|
    ...
    only when you are registering or updating a
  ...
  service_role_arn: string option;
   (**{|
    ...
    up maintenance windows} in the in the {i Amazon Web Services Systems
    Manager User Guide}.|}*)
...
type update_maintenance_window_target_request = {
  replace: bool option;
   (**{|
    If [True], then all fields that are required by the
    {{:  }RegisterTargetWithMaintenanceWindow} operation are also required
    for this API request. Optional fields that aren't specified are set to
    null.|}*)
...
type update_maintenance_window_result = {
  ...
  schedule_timezone: string option;
   (**{|
    ...
    the {{: https://www.iana.org/time-zones }Time Zone Database} on the IANA
    website.|}*)
...
(**{|
    ...
    Manager, you can apply tags to Systems Manager documents (SSM documents),
    managed nodes, maintenance windows, parameters, patch baselines,
...
type document_description = {
  ...
  target_type: string option;
   (**{|
    ...
    on. For example, [/AWS::EC2::Instance]. For a list of valid resource
    types, see
  ...
  status_information: string option;
   (**{|
    ...
    the [StatusInformation] message, "The specified S3 bucket doesn't exist.
...
type attachments_source = {
  ...
  values: string list option;
   (**{|
    ...
    attachment to a document. The format for {b Value} depends on the type of
    key you specify.
    
    {ol 
          {- For the key {i SourceUrl}, the value is an S3 bucket location.
             For example:
             
          ["Values": [ "s3://doc-example-bucket/my-folder" ]]
          }
          
          {- For the key {i S3FileUrl}, the value is a file in an S3 bucket.
             For example:
             
          ["Values": [ "s3://doc-example-bucket/my-folder/my-file.py" ]]
          }
          
          {- For the key {i AttachmentReference}, the value is constructed
             from the name of another SSM document in your account, a version
             number of that document, and a file attached to that document
...
type update_document_request = {
  ...
  document_version: string option;
   (**{|
    ...
    can specify the version number of the latest version or use the [$LATEST]
    variable.
...
type invalid_document = {
  message: string option;
   (**{|
    The SSM document doesn't exist or the document isn't available to the
    user. This exception can be issued by various API operations.|}*)
...
type association_description = {
  ...
  duration: int option;
   (**{|
    ...
    running are cancelled, and no pending executions are started on remaining
    targets.|}*)
  ...
  sync_compliance: association_sync_compliance option;
   (**{|
    ...
    execution to determine the compliance status. If the association
    execution runs successfully, then the association is [COMPLIANT]. If the
    association execution doesn't run successfully, the association is
    [NON-COMPLIANT].
    
    In [MANUAL] mode, you must specify the [AssociationId] as a parameter for
    the {{:  }PutComplianceItems} API operation. In this case, compliance
    data isn't managed by State Manager, a capability of Amazon Web Services
  ...
  max_concurrency: string option;
   (**{|
    ...
    is allowed to run. During the next association interval, the new managed
    node will process its association within the limit specified for
    [MaxConcurrency].|}*)
...
type update_association_request = {
   ...
   (**{|
    ...
    {ol 
          ...
          {- The command specifies the
             ...
             parameter, which means that the association doesn't run
             immediately after it is updated, but only according to the
             specified schedule.
             }
  ...
  schedule_offset: int option;
   (**{|
    ...
    example, if you specified a cron schedule of [cron(0 0 ? * THU#2 *)], you
    could specify an offset of 3 to run the association each Sunday after the
    second Thursday of the month. For more information about cron schedules
    for associations, see
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/reference-cron-and-rate-expressions.html }Reference:
    Cron and rate expressions for Systems Manager} in the {i Amazon Web
    Services Systems Manager User Guide}.
    
    To use offsets, you must specify the [ApplyOnlyAtCronInterval] parameter.
    This option tells the system not to run an association immediately after
    you create it.
    |}*)
  ...
  calendar_names: string list option;
   (**{|
    ...
    run when that change calendar is open. For more information, see
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-change-calendar }Amazon
    Web Services Systems Manager Change Calendar}.|}*)
  ...
  document_version: string option;
   (**{|
    ...
    Manager always runs the [default] version of a document if shared from
    another account, even though the Systems Manager console shows that a new
    version was processed. If you want to run an association using a new
    version of a document shared form another account, you must set the
...
(**{|
    ...
    Systems Manager or you might not have permission to perform the
    operation.|}*)
type invalid_target = {
  message: string option;
...
(**{|
    ...
    IDs. For example, you sent an document for a Windows managed node to a
    Linux node.|}*)
...
(**{|
    The [Context] attribute that you specified for the [InventoryItem] isn't
    allowed for this inventory type. You can only use the [Context] attribute
...
(**{|
    ...
    on the advanced-instances tier} in the {i Amazon Web Services Systems
    Manager User Guide}.|}*)
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-getting-started.html }Getting
    started with Session Manager} in the {i Amazon Web Services Systems
    Manager User Guide}. This error is also returned if you attempt to start
    a session on a managed node that is located in a different account or
    Region|}*)
...
type start_session_response = {
  stream_url: string option;
   (**{|
    ...
    {b region} values, see the {b Region} column in
    {{: https://docs.aws.amazon.com/general/latest/gr/ssm.html#ssm_region }Systems
    Manager service endpoints} in the {i Amazon Web Services General
    Reference}.
...
type start_change_request_execution_request = {
  ...
  auto_approve: bool option;
   (**{|
    ...
    state of an associated calendar is [CLOSED], change freeze approvers must
    still grant permission for this change request to run. If they don't, the
    change won't be processed until the calendar state is again [OPEN].
    |}*)
...
(**{|
    ...
    incorrect. For example, they may not match the set of parameters
...
type start_automation_execution_request = {
  ...
  target_locations: target_location list option;
   (**{|
    ...
    Web Services accounts} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
   ...
   (**{|
    ...
    {ol 
          {- InvokedAfter: Specify a timestamp to limit your results. For
             example, specify 2018-08-29T00:00:00Z to see sessions that
             started August 29, 2018, and later.
             }
          
          {- InvokedBefore: Specify a timestamp to limit your results. For
             example, specify 2018-08-29T00:00:00Z to see sessions that
             started before August 29, 2018.
             }
...
(**{|
    ...
    or a feature of a service. For example, if an Amazon Web Services service
    ...
    "false". This means the user can't use this feature unless they change
    ...
    Or, use the {{:  }ResetServiceSetting} to change the value back to the
  ...
  status: string option;
   (**{|
    ...
    {ol 
          ...
          {- PendingUpdate: The current setting uses a default or custom
...
type command = {
  ...
  max_errors: string option;
   (**{|
    ...
    Web Services Systems Manager Run Command} in the {i Amazon Web Services
    Systems Manager User Guide}.|}*)
  ...
  status_details: string option;
   (**{|
    ...
    command statuses} in the {i Amazon Web Services Systems Manager User
    ...
    {ol 
          ...
          {- Success: The command successfully ran on all invocations. This
             is a terminal state.
             }
          ...
          {- Failed: The value of MaxErrors or more command invocations shows
             a status of Failed. This is a terminal state.
             }
          
          {- Incomplete: The command was attempted on all managed nodes and
             one or more invocations doesn't have a value of Success but not
          ...
          {- Rate Exceeded: The number of managed nodes targeted by the
             command exceeded the account limit for pending invocations. The
             system has canceled the command before running it on any managed
  ...
  expires_after: float option;
   (**{|
    ...
    command timeout values} in the {i Amazon Web Services Systems Manager
    User Guide}.|}*)
...
type send_command_request = {
  ...
  cloud_watch_output_config: cloud_watch_output_config option;
   (**{|
    ...
    Amazon CloudWatch Logs. Run Command is a capability of Amazon Web
  ...
  service_role_arn: string option;
   (**{|
    ...
    topic. For information about creating and using this service role, see
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/monitoring-sns-notifications.html }Monitoring
    Systems Manager status changes using Amazon SNS notifications} in the
    {i Amazon Web Services Systems Manager User Guide}.|}*)
  ...
  max_errors: string option;
   (**{|
    ...
    error controls} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
  ...
  max_concurrency: string option;
   (**{|
    ...
    concurrency controls} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
  ...
  document_version: string option;
   (**{|
    ...
    Command Line Interface (Amazon Web Services CLI), then you must escape
    the first two options by using a backslash. If you specify a version
  ...
  targets: target list option;
   (**{|
    ...
    useful when you want to send a command to a large number of managed nodes
    at once. Using [Targets], which accepts tag key-value pairs to identify
    ...
    commands at scale} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
...
(**{|
    The {{:  }PutResourcePolicy} API action enforces two limits. A policy
    can't be greater than 1024 bytes in size. And only one policy can be
...
(**{|
    ...
    role is automatically created when a user creates a resource data sync in
...
(**{|
    ...
    of execution (for example, [Command]), and the date/time of the execution
    using a datetime object that is saved in the following format:
...
type remove_tags_from_resource_request = {
  ...
  resource_id: string;
   (**{|
    ...
    Resource Name (ARN) for the object. Specifically, [ResourceID] is created
    from the strings that come after the word [opsmetadata] in the ARN. For
    ...
    managed nodes. Specify the name of the managed node in the following
  ...
  resource_type: resource_type_for_tagging;
   (**{|
    ...
    The [ManagedInstance] type for this API operation is only for on-premises
    managed nodes. Specify the name of the managed node in the following
    format: [mi-{i ID_number}]. For example, [mi-1a2b3c4d5e6f].
    |}*)
...
type register_target_with_maintenance_window_request = {
  ...
  targets: target list;
   (**{|
    ...
    Register targets with a maintenance window} in the {i Amazon Web Services
    Systems Manager User Guide}.|}*)
...
type put_parameter_request = {
  data_type: string option;
   (**{|
    ...
    which indicates a successful [PutParameter] call for all cases except for
    data type [aws:ec2:image]. If you call [PutParameter] with
    ...
    [aws:ec2:image] value is validated asynchronously, and the [PutParameter]
    ...
    up notifications or trigger actions based on Parameter Store events}. For
    more information about AMI format validation , see
  ...
  tier: parameter_tier option;
   (**{|
    ...
    parameter tiers} in the {i Amazon Web Services Systems Manager User
    Guide}.
    ...
    you don't specify a tier in a request, Parameter Store assigns the tier
    ...
    {ol 
          {- {b Advanced}: With this option, Parameter Store evaluates all
             requests as advanced parameters.
             }
          
          {- {b Intelligent-Tiering}: With this option, Parameter Store
    ...
    a default parameter tier} in the {i Amazon Web Services Systems Manager
    User Guide}.|}*)
  ...
  key_id: string option;
   (**{|
    ...
    {ol 
          {- To use a custom KMS key, choose the [SecureString] data type
  ...
  value: string;
   (**{|
    ...
    parameters. You can't include [{{}}] or [{{ssm:{i parameter-name}}}] in a
    parameter value.
    |}*)
  ...
  name: string;
   (**{|
    ...
    Systems Manager parameters} in the {i Amazon Web Services Systems Manager
    User Guide}.
    ...
    includes the characters in the ARN that precede the name you specify,
    such as [arn:aws:ssm:us-east-2:111122223333:parameter/].
    |}*)
...
(**{|
    ...
    the oldest version has a {i label} attached to it, Parameter Store won't
    ...
    {i parameter-name} by calling the PutParameter API with the overwrite
    flag. Version {i version-number}, the oldest version, can't be deleted
    ...
    parameters, first move the label from the oldest version of the parameter
    to a newer one for use in your operations. For information about moving
    parameter labels, see
    ...
    a parameter label (CLI)} in the {i Amazon Web Services Systems Manager
    User Guide}.|}*)
...
(**{|
    ...
    and constraints for parameter names} in the {i Amazon Web Services
    Systems Manager User Guide}.|}*)
type hierarchy_level_limit_exceeded_exception = {
  message: string option;
   (**{|
    ...
    requirements and constraints for parameter names} in the {i Amazon Web
    Services Systems Manager User Guide}.|}*)
...
type put_compliance_items_request = {
  upload_type: compliance_upload_type option;
   (**{|
    ...
    [SyncCompliance] set to [MANUAL]. By default, all requests use [COMPLETE]
    mode.
...
(**{|
    ...
    on the API operation that includes the filter. Patch Manager API
...
type patch_compliance_data = {
  ...
  state: patch_compliance_data_state;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-compliance-about.html#sysman-compliance-monitor-patch }About
    patch compliance} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
...
type parameter_string_filter = {
  ...
  option_: string option;
   (**{|
    ...
    [Path], valid options include [Recursive] and [OneLevel].)
  ...
  key: string;
   (**{|
    ...
    The [ParameterStringFilter] object is used by the
    {{:  }DescribeParameters} and {{:  }GetParametersByPath} API operations.
    ...
    For [GetParametersByPath], the following patterns listed for [Key] aren't
    valid: [tag], [DataType], [Name], [Path], and [Tier].
    ...
    for Systems Manager parameters} in the {i Amazon Web Services Systems
    Manager User Guide}.|}*)
...
type parameter_inline_policy = {
  policy_status: string option;
   (**{|
    ...
    (the policy hasn't been enforced or applied yet), Finished (the policy
    was applied), Failed (the policy wasn't applied), or InProgress (the
...
type parameter = {
  ...
  value: string option;
   (**{|
    ...
    If type is [StringList], the system returns a comma-separated string with
    no spaces between commas in the [Value] field.
    |}*)
...
type parameter_history = {
  ...
  policies: parameter_inline_policy list option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-policies.html }Assigning
    parameter policies} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
...
(**{|
    ...
    Application Manager. The maximum is 200 OpsMetadata objects. Delete one
    or more OpsMetadata object and try again.|}*)
...
(**{|
    ...
    Manager OpsCenter to view, investigate, and remediate operational work
    items (OpsItems) impacting the performance and health of their Amazon Web
    Services resources. OpsCenter is integrated with Amazon EventBridge and
    Amazon CloudWatch. This means you can configure these services to
    automatically create an OpsItem in OpsCenter when a CloudWatch alarm
    enters the ALARM state or when EventBridge processes an event from any
    Amazon Web Services service that publishes events. Configuring Amazon
    CloudWatch alarms and EventBridge events to automatically create OpsItems
    allows you to quickly diagnose and remediate issues with Amazon Web
    Services resources from a single console.
    ...
    that generated the OpsItem, alarm or event details, alarm history, and an
    alarm timeline graph. For the Amazon Web Services resource, OpsCenter
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter.html }Amazon
    Web Services Systems Manager OpsCenter} in the {i Amazon Web Services
    Systems Manager User Guide}.|}*)
type ops_item = {
  ops_item_arn: string option;
   (**{|
...
type modify_document_permission_request = {
  ...
  account_ids_to_remove: string list option;
   (**{|
    ...
    IDs or {i All}. This action has a higher priority than [AccountIdsToAdd].
  ...
  account_ids_to_add: string list option;
   (**{|
    ...
    The account IDs can either be a group of account IDs or {i All}.|}*)
...
(**{|
    The permission type isn't supported. {i Share} is the only supported
    permission type.|}*)
...
type maintenance_window_task = {
  ...
  targets: target list option;
   (**{|
    ...
    using [Key=instanceids,Values=,]. Tags are specified using
    [Key=,Values=].|}*)
  ...
  task_arn: string option;
   (**{|
    ...
    Manager (SSM document) name or ARN. For [LAMBDA] tasks, it's the function
...
(**{|
    ...
    as part of a task execution performed as part of a maintenance window
    execution.|}*)
...
type list_resource_data_sync_request = {
  ...
  sync_type: string option;
   (**{|
    ...
    an Amazon S3 bucket. Specify [SyncFromSource] to view resource data syncs
...
type inventory_filter = {
  type_: inventory_query_operator_type option;
   (**{|
    ...
    see
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-inventory-aggregate.html }Aggregating
    inventory data} in the {i Amazon Web Services Systems Manager User
    Guide}.
    |}*)
...
(**{|
    ...
    You can also use the [TargetType] Amazon Web Services-provided key. For a
    list of valid resource type values that can be used with this key, see
    {{: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-template-resource-type-ref.html }Amazon
    Web Services resource and property types reference} in the
    {i CloudFormation User Guide}.
  ...
  value: string;
   (**{|
    ...
    {ol 
          ...
          {- {b InvokedBefore}: Specify a timestamp to limit your results.
             For example, specify [2021-07-07T00:00:00Z] to see a list of
             command executions from before July 7, 2021.
             }
          
          {- {b Status}: Specify a valid command status to see a list of all
             command executions with that status. The status choices depend
             on the API you call.
          ...
          {ol 
                ...
                {- [TimedOut] (this includes both Delivery and Execution time
                   outs)
                   }
          ...
          {- {b DocumentName}: Specify name of the Amazon Web Services
             Systems Manager document (SSM document) for which you want to
             see command execution results. For example, specify
             [AWS-RunPatchBaseline] to see command executions that used this
             SSM document to perform security patching operations on managed
             nodes.
             }
   ...
   (**{|
    ...
    {ol 
          ...
          {- Delivery Timed Out: The command wasn't delivered to the managed
             node before the delivery timeout expired. Delivery timeouts
             don't count against the parent command's [MaxErrors] limit, but
             they do contribute to whether the parent command status is
          ...
          {- Failed: The command wasn't successful on the managed node. For a
             ...
             or more plugins wasn't zero. Invocation failures count against
  ...
  status_details: string option;
   (**{|
    ...
    {ol 
          ...
          {- Undeliverable: The command can't be delivered to the managed
             node. The managed node might not exist or might not be
             responding. Undeliverable invocations don't count against the
             parent command's MaxErrors limit and don't contribute to whether
...
type label_parameter_version_result = {
  ...
  invalid_labels: string list option;
   (**{|
    ...
    with parameter labels} in the {i Amazon Web Services Systems Manager User
    Guide}.|}*)
...
(**{|
    ...
    invocations. Verify the command ID and the managed node ID and try again.|}*)
...
type inventory_deletion_status_item = {
  ...
  deletion_summary: inventory_deletion_summary option;
   (**{|
    ...
    summary, see
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-inventory-custom.html#sysman-inventory-delete }Understanding
    the delete inventory summary} in the {i Amazon Web Services Systems
    Manager User Guide}.|}*)
...
type instance_property = {
  ...
  name: string option;
   (**{|
    The value of the EC2 [Name] tag associated with the node. If a [Name] tag
...
type instance_patch_state = {
  ...
  critical_non_compliant_count: int option;
   (**{|
    ...
    were installed but awaiting a required managed node reboot. The status of
   ...
   (**{|
    ...
    {ol 
          {- [RebootIfNeeded]: Patch Manager tries to reboot the managed node
             if it installed any patches, or if any patches are detected with
             a status of [InstalledPendingReboot].
             }
  ...
  install_override_list: string option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/patch-manager-about-aws-runpatchbaseline.html }About
    the [AWS-RunPatchBaseline SSM document]} in the {i Amazon Web Services
    Systems Manager User Guide}.|}*)
...
(**{|
    ...
    {b Example}: To filter for all managed nodes in a patch group having more
    than three patches with a [FailedCount] status, use the following for the
    filter:
...
type instance_information_string_filter = {
  ...
  key: string;
   (**{|
    ...
    SourceIds | SourceTypes | "tag-key" | "tag:[{keyname}]
...
type instance_information = {
  ...
  name: string option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-install-managed-linux.html }Install
    SSM Agent for a hybrid and multicloud environment (Linux)} and
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-install-managed-win.html }Install
    SSM Agent for a hybrid and multicloud environment (Windows)}. To retrieve
    the [Name] tag of an EC2 instance, use the Amazon EC2 [DescribeInstances]
  ...
  iam_role: string option;
   (**{|
    ...
    role for an EC2 instance, use the Amazon EC2 [DescribeInstances]
    operation. For information, see
    {{: https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeInstances.html }DescribeInstances}
    in the {i Amazon EC2 API Reference} or
    {{: https://docs.aws.amazon.com/cli/latest/reference/ec2/describe-instances.html }describe-instances}
  ...
  is_latest_version: bool option;
   (**{|
    ...
    Linux managed node. This field doesn't indicate whether or not the latest
    version is installed on Windows managed nodes, because some older
...
(**{|
    ...
    method. The [InstanceInformationFilterList] method is a legacy method and
...
type get_parameters_request = {
  ...
  names: string list;
   (**{|
    ...
    The results for [GetParameters] requests are listed in alphabetical order
    in query responses.
    ...
    with shared parameters} in the {i Amazon Web Services Systems Manager
    User Guide}.|}*)
...
type get_parameter_request = {
  ...
  name: string;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/sharing.html }Working
    with shared parameters} in the {i Amazon Web Services Systems Manager
    User Guide}.|}*)
...
type get_maintenance_window_task_result = {
  ...
  max_errors: string option;
   (**{|
    ...
    For maintenance window tasks without a target specified, you can't supply
    a value for this option. Instead, the system inserts a placeholder value
    of [1], which may be reported in the response to this command. This value
...
type get_maintenance_window_execution_task_invocation_result = {
  ...
  owner_information: string option;
   (**{|
    ...
    Amazon EventBridge events raised while running tasks for these targets in
    this maintenance window.|}*)
...
type get_deployable_patch_snapshot_for_instance_result = {
  product: string option;
   (**{|
    ...
    Amazon Linux 2015.09) on the managed node for the specified patch
    snapshot.|}*)
...
type get_command_invocation_result = {
  ...
  standard_output_url: string option;
   (**{|
    The URL for the complete text written by the plugin to [stdout] in Amazon
    Simple Storage Service (Amazon S3). If an S3 bucket wasn't specified,
   ...
   (**{|
    ...
    {ol 
          ...
          {- Delayed: The system attempted to send the command to the target,
             ...
             available because of network issues, because the node was
          ...
          {- Execution Timed Out: The command started to run on the managed
             ...
             expired. Execution timeouts count against the [MaxErrors] limit
          ...
          {- Failed: The command wasn't run successfully on the managed node.
             ...
             For a command invocation, this indicates that the result code
             for one or more plugins wasn't zero. Invocation failures count
          ...
          {- Terminated: The parent command exceeded its [MaxErrors] limit
             and subsequent command invocations were canceled by the system.
             This is a terminal state.
             }
  ...
  response_code: int option;
   (**{|
    ...
    is [-1], then the command hasn't started running on the managed node, or
    it wasn't received by the node.|}*)

  plugin_name: string option;
   (**{|
    The name of the plugin, or {i step name}, for which details are reported.
...
type get_command_invocation_request = {
  plugin_name: string option;
   (**{|
    ...
    the step you want details for. Alternatively, use
...
type get_calendar_state_response = {
  ...
  state: calendar_state option;
   (**{|
    ...
    allowed to proceed, and a [CLOSED] calendar indicates that actions aren't
    allowed to proceed.|}*)
   ...
   (**{|
    ...
    {ol 
          ...
          {- The command specifies the
             ...
             parameter, which means that the association doesn't run
             immediately after it is created, but only according to the
             specified schedule.
             }
...
type describe_patch_group_state_result = {
  ...
  instances_with_critical_non_compliant_patches: int option;
   (**{|
    ...
    rejected, or were installed but awaiting a required managed node reboot.
    The status of these managed nodes is [NON_COMPLIANT].|}*)
  ...
  instances_with_unreported_not_applicable_patches: int option;
   (**{|
    ...
    supported limit, which aren't reported by name to Inventory. Inventory is
...
type describe_parameters_request = {
  shared: bool option;
   (**{|
    ...
    Resource Share Created From Policy] must have been promoted to a standard
...
type describe_maintenance_window_tasks_request = {
  ...
  filters: maintenance_window_filter list option;
   (**{|
    ...
    supported filter keys are [WindowTaskId], [TaskArn], [Priority], and
    [TaskType].|}*)
...
type describe_maintenance_window_executions_request = {
  ...
  filters: maintenance_window_filter list option;
   (**{|
    ...
    {ol 
          {- Key. A string between 1 and 128 characters. Supported keys
             include [ExecutedBefore] and [ExecutedAfter].
             }
  ...
  filters: patch_orchestrator_filter list option;
   (**{|
    ...
    {ol 
          ...
          {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/patch-manager-compliance-states.html }Understanding
          patch compliance state values} in the {i Amazon Web Services
          Systems Manager User Guide}.
          }
...
type describe_instance_information_request = {
  ...
  max_results: int option;
   (**{|
    The maximum number of items to return for this call. The call also
    returns a token that you can specify in a subsequent call to get the next
    set of results. The default value is 10 items.|}*)
...
type describe_document_permission_response = {
  ...
  account_ids: string list option;
   (**{|
    ...
    either an Amazon Web Services account or {i All}.|}*)
...
type automation_execution_metadata = {
  ...
  automation_type: automation_type option;
   (**{|
    ...
    accounts} in the {i Amazon Web Services Systems Manager User Guide}.|}*)
...
(**{|
    ...
    servers or VMs using Run Command. A server or VM that has been registered
    with Amazon Web Services Systems Manager is called a managed node.|}*)
type activation = {
  tags: tag list option;
   (**{|
...
type delete_inventory_result = {
  deletion_summary: inventory_deletion_summary option;
   (**{|
    ...
    summary, see
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-inventory-custom.html#sysman-inventory-delete-summary }Understanding
    the delete inventory summary} in the {i Amazon Web Services Systems
    Manager User Guide}.|}*)
  ...
  deletion_id: string option;
   (**{|
    ...
    completed before you begin other operations.|}*)
...
type delete_document_request = {
  force: bool option;
   (**{|
    ...
    you can delete the document. For example, you must specify a [Force] flag
    to delete a document of type [ApplicationConfigurationSchema]. You can
    restrict access to the [Force] flag in an Identity and Access Management
    (IAM) policy.|}*)
...
type create_resource_data_sync_request = {
  ...
  sync_type: string option;
   (**{|
    ...
    multiple Regions, or multiple Amazon Web Services accounts and Amazon Web
    Services Regions, as listed in Organizations for Explorer. If you specify
    [SyncFromSource], you must provide a value for [SyncSource]. The default
...
type create_document_request = {
  ...
  version_name: string option;
   (**{|
    ...
    with the document. For example, [Release12.1]. This value is unique
  ...
  display_name: string option;
   (**{|
    ...
    update this value at a later time using the {{:  }UpdateDocument}
    operation.|}*)
  ...
  requires: document_requires list option;
   (**{|
    ...
    validation purposes. In this case, an [ApplicationConfiguration] document
    requires an [ApplicationConfigurationSchema] document for validation
    purposes. For more information, see
    {{: https://docs.aws.amazon.com/appconfig/latest/userguide/what-is-appconfig.html }What
    is AppConfig?} in the {i AppConfig User Guide}.|}*)
...
type create_association_request = {
  ...
  sync_compliance: association_sync_compliance option;
   (**{|
    ...
    In [MANUAL] mode, you must specify the [AssociationId] as a parameter for
    the {{:  }PutComplianceItems} API operation. In this case, compliance
    data isn't managed by State Manager. It is managed by your direct call to
  ...
  targets: target list option;
   (**{|
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-state-manager-targets-and-rate-controls.html }About
    targets and rate controls in State Manager associations} in the {i Amazon
...
type create_activation_request = {
  ...
  tags: tag list option;
   (**{|
    ...
    to add tags to your managed nodes, see {{:  }AddTagsToResource}. For
...
type add_tags_to_resource_request = {
  ...
  resource_id: string;
   (**{|
    ...
    following format: [mi-{i ID_number}]. For example, [mi-1a2b3c4d5e6f].
    |}*)
...
(**{|
    ...
    Web Services applications and resources and a secure end-to-end
    ...
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/ }Amazon
    Web Services Systems Manager User Guide}. To get started, see
    {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-setting-up.html }Setting
    ...
    {ol 
          {- For information about each of the capabilities that comprise
             Systems Manager, see
             {{: https://docs.aws.amazon.com/systems-manager/latest/userguide/what-is-systems-manager.html#systems-manager-capabilities }Systems
             Manager capabilities} in the {i Amazon Web Services Systems
             Manager User Guide}.
             }
          
          {- For details about predefined runbooks for Automation, a
             capability of Amazon Web Services Systems Manager, see the
             {i {{: https://docs.aws.amazon.com/systems-manager-automation-runbooks/latest/userguide/automation-runbook-reference.html }Systems
             Manager Automation runbook reference}}.
             }
          
          {- For information about AppConfig, a capability of Systems
             Manager, see the
             {i {{: https://docs.aws.amazon.com/appconfig/latest/userguide/ }AppConfig
             User Guide}} and the
             {i {{: https://docs.aws.amazon.com/appconfig/2019-10-09/APIReference/ }AppConfig
             API Reference}}.
             }
          
          {- For information about Incident Manager, a capability of Systems
             Manager, see the
             {i {{: https://docs.aws.amazon.com/incident-manager/latest/userguide/ }Systems
             Manager Incident Manager User Guide}} and the
             {i {{: https://docs.aws.amazon.com/incident-manager/latest/APIReference/ }Systems
