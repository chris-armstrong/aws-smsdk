...
type untag_resource_request = {
  ...
  resource_ar_n: string;
   (**{|
    ...
    creating an ARN, see
    {{: https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_Tagging.ARN.html#USER_Tagging.ARN.Constructing }Constructing
   ...
   (**{|
    ...
    {ol 
          ...
          {- The counter metrics listed in
             ...
             Insights operating system counters} in the {i Amazon Aurora User
             Guide}.
             }
          ...
          {- The counter metrics listed in
             ...
             Insights operating system counters} in the {i Amazon RDS User
             Guide}.
             }
    ...
    raw values, and [db.sampledload.avg] less than [db.load.avg]. For most
...
type recommendation = {
  recommendation_description: string option;
   (**{|
    ...
    example, [Investigate the following SQLs that contributed to 100% of the
    total DBLoad during that time period: sql-id]|}*)
...
type list_performance_analysis_reports_request = {
  ...
  identifier: string;
   (**{|
    ...
    source. In the console, the identifier is shown as {i ResourceID}. When
  ...
  metric_types: string list;
   (**{|
    ...
    {ol 
          ...
          {- [db] (DB load metrics) - All engines except for Amazon
             DocumentDB
             }
...
type dimension_group = {
  ...
  dimensions: string list option;
   (**{|
    ...
    is not present, then it signifies that all of the dimensions in the group
    ...
    {ol 
          ...
          {- [db.sql_tokenized.id] - The hash of the SQL digest generated by
             ...
             because Amazon Web Services Support can look at this data to
             help you troubleshoot database issues.
             }
          
          {- [db.sql_tokenized.db_id] - Either the native database ID used to
             refer to the SQL statement, or a synthetic ID such as
             [pi-2372568224] that Performance Insights generates if the
             native database ID isn't available (all engines except Amazon
             DocumentDB).
             }
          ...
          {- [db.wait_event.name] - The event for which the backend is
             waiting (all engines except Amazon DocumentDB).
             }
          
          {- [db.wait_event.type] - The type of event for which the backend
  ...
  group: string;
   (**{|
    ...
    {ol 
          {- [db] - The name of the database to which the client is
          ...
          {- [db.query_tokenized] - The digest query (only Amazon
             DocumentDB).
             }
...
(**{|
    ...
    append an aggregate function to the metric. For example, to find the
    average for the metric [db.load] you must use [db.load.avg]. Valid values
    for aggregate functions include [.avg], [.min], [.max], and [.sum]. If no
    other parameters are specified, Performance Insights returns all data
    points for the specified metric. Optionally, you can request that the
    data points be aggregated by dimension group ([GroupBy]), and return only
...
type metric_query = {
  ...
  group_by: dimension_group option;
   (**{|
    ...
    You must specify a valid dimension group. Performance Insights will
    return all of the dimensions within that group, unless you provide the
...
type get_resource_metrics_request = {
  ...
  start_time: float;
   (**{|
    ...
    can extend this range up to 2 years. The value specified is
  ...
  metric_queries: metric_query list;
   (**{|
    ...
    can provide filtering criteria. You must append the aggregate function to
    the metric. For example, to find the average for the metric [db.load] you
    must use [db.load.avg]. Valid values for aggregate functions include
    [.avg], [.min], [.max], and [.sum].|}*)
...
(**{|
    ...
    Insight that were calculated and included in text and annotations on the
    DB load chart.|}*)
type data = {
...
type insight = {
  ...
  recommendations: recommendation list option;
   (**{|
    List of recommendations for the insight. For example, [Investigate the
    following SQLs that contributed to 100% of the total DBLoad during that
    time period: sql-id].|}*)

  description: string option;
   (**{|
    Description of the insight. For example: [A high severity Insight found
    between 02:00 to 02:30, where there was an unusually high DB load 600x
...
type get_dimension_key_details_request = {
  requested_dimensions: string list option;
   (**{|
    ...
    Insights returns all dimension data within the specified dimension group.
    Specify dimension names for the following dimension groups:
  ...
  identifier: string;
   (**{|
    ...
    must be immutable and unique within an Amazon Web Services Region. When a
    DB instance is the data source, specify its [DbiResourceId] value. For
...
type describe_dimension_keys_response = {
  ...
  aligned_start_time: float option;
   (**{|
    ...
    will be less than or equal to the value of the user-specified
    [StartTime].|}*)
...
type describe_dimension_keys_request = {
  ...
  additional_metrics: string list option;
   (**{|
    ...
    dimension group in the [GroupBy] parameter is [db.sql_tokenized], you can
    specify per-SQL metrics to get the values for the top [N] SQL digests.
    The response syntax is as follows: ["AdditionalMetrics" : { "{i string}"
    : "{i string}" }].|}*)

  group_by: dimension_group;
   (**{|
    A specification for how to aggregate the data points from a query result.
    You must specify a valid dimension group. Performance Insights returns
    all dimensions within this group, unless you provide the names of
    specific dimensions within this group. You can also request that
    Performance Insights return a limited number of values for a dimension.|}*)
  ...
  period_in_seconds: int option;
   (**{|
    ...
    If you don't specify [PeriodInSeconds], then Performance Insights chooses
...
(**{|
    ...
    {ol 
          {- To learn more about Performance Insights and Amazon Aurora DB
             instances, go to the
             {i {{: https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_PerfInsights.html }Amazon
             Aurora User Guide}}.
             }
          
          {- To learn more about Performance Insights and Amazon RDS DB
             instances, go to the
             {i {{: https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_PerfInsights.html }Amazon
             RDS User Guide}}.
             }
          
          {- To learn more about Performance Insights and Amazon DocumentDB
             clusters, go to the
             {i {{: https://docs.aws.amazon.com/documentdb/latest/developerguide/performance-insights.html }Amazon
