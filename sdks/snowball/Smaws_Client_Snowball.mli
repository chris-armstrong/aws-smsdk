...
(**{|
    ...
    associated with a specific job. The [Notification] object is returned as
    a part of the response syntax of the [DescribeJob] action in the
    ...
    have Amazon SNS notifications sent out for all job states with
...
(**{|
    ...
    device that your transferred data will be exported from or imported into.
    Amazon Web Services Snow Family supports Amazon S3 and NFS (Network File
    System).|}*)
...
(**{|
    ...
    this object can have an optional [KeyRange] value. The length of the
...
type s3_on_device_service_configuration = {
  fault_tolerance: int option;
   (**{|
    ...
    additional input helps when the specified [StorageLimit] matches more
...
type update_job_request = {
  ...
  resources: job_resource option;
   (**{|
    The updated [JobResource] object, or the updated {{:  }JobResource}
    object.|}*)
...
(**{|
    ...
    error occurred. Check the address with your region's carrier and try
...
type shipping_details = {
  ...
  inbound_shipment: shipment option;
   (**{|
    The [Status] and [TrackingNumber] values for a Snow device being returned
    to Amazon Web Services for a particular job.|}*)

  shipping_option: shipping_option option;
   (**{|
    The shipping speed for a particular job. This speed doesn't dictate how
    soon you'll get the Snow device from the job's creation date. This speed
...
type job_list_entry = {
  ...
  is_master: bool option;
   (**{|
    ...
    with any Snowballs. Instead, each main job will have at least one job
    part, and each job part is associated with a Snowball. It might take some
    time before the job parts associated with a particular main job are
...
type list_jobs_result = {
  ...
  job_list_entries: job_list_entry list option;
   (**{|
    Each [JobListEntry] object contains a job's state, a job's ID, and a
    value that indicates whether the job is a job part, in the case of export
    jobs.|}*)
...
(**{|
    ...
    can look at the two associated logs: a success log and a failure log. The
    logs are saved in comma-separated value (CSV) format, and the name of
    each log includes the ID of the job or job part that the log describes.|}*)
type job_logs = {
...
type job_metadata = {
  ...
  remote_management: remote_management option;
   (**{|
    ...
    at your location. Otherwise, you need to use the Snowball Client to
    manage the device.|}*)
...
type get_software_updates_result = {
  updates_ur_i: string option;
   (**{|
    ...
    specified [JobId] value. The software update will be available for 2 days
    after this request is made. To access an update after the 2 days have
    passed, you'll have to make another call to [GetSoftwareUpdates].|}*)
...
type create_job_request = {
  ...
  remote_management: remote_management option;
   (**{|
    ...
    outside of your internal network. When set to [INSTALLED_AUTOSTART],
    remote management will automatically be available when the device arrives
    at your location. Otherwise, you need to use the Snowball Edge client to
    manage the device. When set to [NOT_INSTALLED], remote management will
  ...
  cluster_id: string option;
   (**{|
    ...
    you need to provide only this [clusterId] value. The other job attributes
  ...
  resources: job_resource option;
   (**{|
    ...
    length of the range by providing either an inclusive [BeginMarker] value,
...
type create_address_result = {
  address_id: string option;
   (**{|
    ...
    when you create a job to specify which address you want the Snow device
...
(**{|
    ...
    information, see the
    {{: https://docs.aws.amazon.com/AWSImportExport/latest/ug/api-reference.html }User
    Guide}.|}*)
