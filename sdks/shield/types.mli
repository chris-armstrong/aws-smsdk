...
type update_subscription_request = {
  auto_renew: auto_renew option;
   (**{|
    ...
    [UpdateSubscription] request. If the [UpdateSubscription] request does
    not included a value for [AutoRenew], the existing value for [AutoRenew]
    remains unchanged.|}*)
...
type update_protection_group_request = {
  ...
  resource_type: protected_resource_type option;
   (**{|
    ...
    this when you set [Pattern] to [BY_RESOURCE_TYPE] and you must not set it
  ...
  pattern: protection_group_pattern;
   (**{|
    ...
    the group. You can include all resources that have protections, provide a
    list of resource Amazon Resource Names (ARNs), or include all resources
    of a specified resource type.|}*)
  ...
  aggregation: protection_group_aggregation;
   (**{|
    ...
    {ol 
          ...
          {- Mean - Use the average of the traffic across the group. This is
             a good choice for resources that share traffic uniformly.
...
type update_emergency_contact_settings_request = {
  emergency_contact_list: emergency_contact list option;
   (**{|
    ...
    (SRT) can use to contact you if you have proactive engagement enabled,
...
type contributor = {
  ...
  name: string option;
   (**{|
    ...
    depends on the [AttackPropertyIdentifier] setting in the [AttackProperty]
...
type protection_group = {
  ...
  pattern: protection_group_pattern;
   (**{|
    The criteria to use to choose the protected resources for inclusion in
    the group. You can include all resources that have protections, provide a
    list of resource ARNs (Amazon Resource Names), or include all resources
    of a specified resource type.|}*)
...
type list_protection_groups_request = {
  inclusion_filters: inclusion_protection_group_filters option;
   (**{|
    ...
    aggregation settings. You can provide up to one criteria per filter type.
    Shield Advanced returns the protection groups that exactly match all of
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use_passrole.html }Granting
...
type attack_property = {
  ...
  attack_property_identifier: attack_property_identifier option;
   (**{|
    ...
    valid only for WordPress reflective pingback events.|}*)
...
type associate_proactive_engagement_details_request = {
  emergency_contact_list: emergency_contact list;
   (**{|
    ...
    retrieve the list using [DescribeEmergencyContactSettings] and then
    provide it here.
    |}*)
...
(**{|
    ...
    {{: https://docs.aws.amazon.com/waf/latest/developerguide/ }WAF and
