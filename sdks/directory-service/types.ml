...
(**{|
    ...
    reached. You can use the {{:  }GetSnapshotLimits} operation to determine
    the snapshot limits for a directory.|}*)
...
type setting_entry = {
  ...
  type_: string option;
   (**{|
    ...
    same type. For example, [Protocol], [Cipher], or [Certificate-Based
    Authentication].|}*)
...
type list_log_subscriptions_result = {
  ...
  log_subscriptions: log_subscription list option;
   (**{|
    A list of active {{:  }LogSubscription} objects for calling the Amazon
...
type list_ip_routes_result = {
  next_token: string option;
   (**{|
    If not null, more results are available. Pass this value for the
    {i NextToken} parameter in a subsequent call to {{:  }ListIpRoutes} to
    retrieve the next set of items.|}*)
...
type list_certificates_request = {
  ...
  next_token: string option;
   (**{|
    ...
    value of the returned [NextToken] element in your request until the token
    comes back as [null]. Pass [null] if this is the first call.|}*)
...
type describe_update_directory_request = {
  next_token: string option;
   (**{|
    The [DescribeUpdateDirectoryResult]. NextToken value from a previous call
...
type directory_description = {
  ...
  access_url: string option;
   (**{|
    The access URL for the directory, such as [http://.awsapps.com]. If no
    alias has been created for the directory, [] is the directory identifier,
...
type describe_directories_result = {
  next_token: string option;
   (**{|
    If not null, more results are available. Pass this value for the
    [NextToken] parameter in a subsequent call to {{:  }DescribeDirectories}
...
type client_authentication_setting_info = {
  ...
  type_: client_authentication_type option;
   (**{|
    ...
    is specified, a list of all client authentication types that are
...
type describe_client_authentication_settings_result = {
  ...
  client_authentication_settings_info: client_authentication_setting_info list option;
   (**{|
    ...
    the status of the client authentication type was last updated, whether
    the client authentication type is enabled or disabled, and the type of
...
type describe_client_authentication_settings_request = {
  ...
  next_token: string option;
   (**{|
    The {i DescribeClientAuthenticationSettingsResult.NextToken} value from a
    previous call to {{:  }DescribeClientAuthenticationSettings}. Pass null
...
(**{|
    ...
    use the {{:  }GetDirectoryLimits} operation to determine your directory
...
type create_alias_request = {
  alias: string;
   (**{|
    ...
    operation throws an [EntityAlreadyExistsException] error if the alias
    already exists.|}*)
...
(**{|
    ...
    {{: http://docs.aws.amazon.com/directoryservice/latest/admin-guide/what_is.html }Directory
