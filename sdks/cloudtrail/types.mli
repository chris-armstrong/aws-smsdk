...
type update_trail_request = {
  is_organization_trail: bool option;
   (**{|
    ...
    behalf of an Amazon Web Services account that is the management account
    for an organization in Organizations. If the trail is not an organization
    trail and this is set to [true], the trail will be created in all Amazon
    Web Services accounts that belong to the organization. If the trail is an
  ...
  s3_bucket_name: string option;
   (**{|
    Specifies the name of the Amazon S3 bucket designated for publishing log
    files. See
    {{: https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html }Amazon
...
(**{|
    ...
    resources that is not valid. The combination of event selectors or
    advanced event selectors and data resources is not valid. A trail can
    have up to 5 event selectors. If a trail uses advanced event selectors, a
    maximum of 500 total values for all conditions in all advanced event
    selectors is allowed. A trail is limited to 250 data resources. These
    data resources can be distributed across event selectors, but the overall
    ...
    {ol 
          ...
          {- Specify a valid number of data resources (1 to 250) for an event
             selector. The limit of number of resources on an individual
             event selector is configurable up to 250. However, this upper
             limit is allowed only if the total number of data resources does
             not exceed 250 across all event selectors for a trail.
             }
          
          {- Specify up to 500 values for all conditions in all advanced
             event selectors for a trail.
             }
          
          {- Specify a valid value for a parameter. For example, specifying
             the [ReadWriteType] parameter with a value of [read-only] is not
             valid.
             }
...
(**{|
    ...
    CloudTrail and Organizations. For more information, see
    ...
    to enable or disable trusted access} in the {i Organizations User Guide}
    and
    {{: https://docs.aws.amazon.com/awscloudtrail/latest/userguide/creating-an-organizational-trail-prepare.html }Prepare
    For Creating a Trail For Your Organization} in the {i CloudTrail User
    Guide}.|}*)
...
type advanced_field_selector = {
  ...
  field: string;
   (**{|
    ...
    Web Services, the field is used only for selecting events as filtering is
    not supported.
    ...
    {ol 
          {- {b [readOnly]} - Optional. Can be set to [Equals] a value of
             [true] or [false]. If you do not add this field, CloudTrail logs
             both [read] and [write] events. A value of [true] logs only
             [read] events. A value of [false] logs only [write] events.
             }
          
          {- {b [eventSource]} - For filtering management events only. This
             can be set to [NotEquals] [kms.amazonaws.com] or [NotEquals]
             [rdsdata.amazonaws.com].
             }
          
          {- {b [eventName]} - Can use any operator. You can use it to
             ﬁlter in or ﬁlter out any data event logged to CloudTrail,
             such as [PutBucket] or [GetSnapshotBlock]. You can have multiple
             values for this ﬁeld, separated by commas.
             }
          
          {- {b [eventCategory]} - This is required and must be set to
             [Equals].
          ...
          {- {b [resources.type]} - This ﬁeld is required for CloudTrail
             data events. [resources.type] can only use the [Equals]
          ...
          of the following formats. To log all data events for all objects in
          a specific S3 bucket, use the [StartsWith] operator, and include
          ...
          When resources.type equals [AWS::AppConfig::Configuration], and the
          operator is set to [Equals] or [NotEquals], the ARN must be in the
          following format:
          ...
          When resources.type equals [AWS::CodeWhisperer::Customization], and
          the operator is set to [Equals] or [NotEquals], the ARN must be in
          the following format:
          ...
          When [resources.type] equals [AWS::GreengrassV2::ComponentVersion],
          ...
          When [resources.type] equals [AWS::IoT::Thing], and the operator is
          ...
          When [resources.type] equals [AWS::ManagedBlockchain::Network], and
          the operator is set to [Equals] or [NotEquals], the ARN must be in
          the following format:
          ...
          When [resources.type] equals [AWS::QApps:QApp], and the operator is
          set to [Equals] or [NotEquals], the ARN must be in the following
          format:
          ...
          When [resources.type] equals [AWS::QBusiness::Application], and the
          operator is set to [Equals] or [NotEquals], the ARN must be in the
          following format:
          ...
          When [resources.type] equals [AWS::SCN::Instance], and the operator
          is set to [Equals] or [NotEquals], the ARN must be in the following
          format:
          ...
          When [resources.type] equals [AWS::SNS::Topic], and the operator is
          set to [Equals] or [NotEquals], the ARN must be in the following
          format:
          ...
          When [resources.type] equals [AWS::SQS::Queue], and the operator is
          set to [Equals] or [NotEquals], the ARN must be in the following
          format:
...
(**{|
    ...
    about advanced event selectors, see
    ...
    data events} in the {i CloudTrail User Guide}.
...
type update_event_data_store_response = {
  ...
  federation_status: federation_status option;
   (**{|
    ...
    query federation} status. The status is [ENABLED] if Lake query
    federation is enabled, or [DISABLED] if Lake query federation is
    disabled. You cannot delete an event data store if the [FederationStatus]
    is [ENABLED].|}*)
  ...
  updated_timestamp: float option;
   (**{|
    ...
    [UpdatedTimestamp] is always either the same or newer than the time shown
    in [CreatedTimestamp].|}*)
...
type update_event_data_store_request = {
   ...
   (**{|
    ...
    {ol 
          {- [EXTENDABLE_RETENTION_PRICING] - This billing mode is generally
             recommended if you want a flexible retention period of up to
             3653 days (about 10 years). The default retention period for
  ...
  kms_key_id: string option;
   (**{|
    ...
    and prevents users from querying the data in the event data store that
    was encrypted with the key. After you associate an event data store with
    a KMS key, the KMS key cannot be removed or changed. Before you disable
    or delete a KMS key that you are using with an event data store, delete
    or back up your event data store.
  ...
  retention_period: int option;
   (**{|
    ...
    example, if you set a retention period of 90 days, CloudTrail will remove
    events when the [eventTime] is older than 90 days.
...
(**{|
    ...
    specified [InsightType] in the [InsightSelectors] statement is not valid.
    Valid values for [InsightType] are [ApiCallRateInsight] and
    ...
    [InsightSelectors] includes an [InsightType] of [ApiCallRateInsight], the
    source event data store must log [write] management events. If the
...
type trail = {
  ...
  name: string option;
   (**{|
    Name of the trail set by calling {{:  }CreateTrail}. The maximum length
    is 128 characters.|}*)
...
type stop_logging_request = {
  name: string;
   (**{|
    ...
    CloudTrail will stop logging Amazon Web Services API calls. The following
...
type stop_import_response = {
  ...
  end_event_time: float option;
   (**{|
    ...
    imported trail events to only those events logged within a specified time
    period.|}*)
...
type start_import_request = {
  ...
  end_event_time: float option;
   (**{|
    ...
    imported trail events to only those events logged within a specified time
    period. When you specify a time range, CloudTrail checks the prefix and
    log file names to verify the names contain a date between the specified
...
(**{|
    ...
    {ol 
          ...
          {- Contains at least one statement. The policy can have a maximum
             of 20 statements.
             }
...
type put_insight_selectors_response = {
  ...
  insight_selectors: insight_selector list option;
   (**{|
    ...
    on a trail or event data store. The valid Insights types are
...
type put_insight_selectors_request = {
  insights_destination: string option;
   (**{|
    ...
    that logs Insights events. To enable Insights on an event data store, you
    must provide both the [EventDataStore] and [InsightsDestination]
    parameters.
  ...
  event_data_store: string option;
   (**{|
    ...
    which you want to change or add Insights selectors. To enable Insights on
    an event data store, you must provide both the [EventDataStore] and
  ...
  insight_selectors: insight_selector list;
   (**{|
    ...
    or event data store. [ApiCallRateInsight] and [ApiErrorRateInsight] are
    valid Insight types.
...
(**{|
    ...
    example, the CloudTrail user specified an empty prefix, and the option to
    ...
    {ol 
          ...
          {- The [PutObject] API operation is an Amazon S3 object-level API.
             ...
             CloudTrail user specified an S3 bucket with an empty prefix,
             events that occur on any object in that bucket are logged. The
          ...
          {- The [PutObject] API operation occurred for an object in an S3
             bucket that the CloudTrail user didn't specify for the trail.
    ...
    {ol 
          {- A user runs a script that includes a call to the
             {i MyLambdaFunction} function and the {i MyOtherLambdaFunction}
             function.
             }
          
          {- The [Invoke] API operation on {i MyLambdaFunction} is an Lambda
             API. It is recorded as a data event in CloudTrail. Because the
             CloudTrail user specified logging data events for
             {i MyLambdaFunction}, any invocations of that function are
             logged. The trail processes and logs the event.
             }
          
          {- The [Invoke] API operation on {i MyOtherLambdaFunction} is an
             ...
             {i MyOtherLambdaFunction} does not match the function specified
...
type data_resource = {
  values: string list option;
   (**{|
    ...
    {ol 
          ...
          {- To log data events for specific objects, specify the S3 bucket
             and object prefix such as
             [arn:aws:s3:::bucket-1/example-images]. The trail logs data
          ...
          This also enables logging of [Invoke] activity performed by any
          user or role in your Amazon Web Services account, even if that
          ...
          {i arn:aws:lambda:us-west-2:111111111111:function:helloworld}. They
  ...
  type_: string option;
   (**{|
    ...
    selectors. For more information about these additional resource types,
    see
...
type event_selector = {
  ...
  data_resources: data_resource list option;
   (**{|
    ...
    specify up to 250 resources for an individual event selector, but the
    total number of data resources cannot exceed 250 across all event
...
type put_event_selectors_request = {
  advanced_event_selectors: advanced_event_selector list option;
   (**{|
    ...
    can use either [AdvancedEventSelectors] or [EventSelectors], but not
...
type lookup_events_response = {
  next_token: string option;
   (**{|
    ...
    'Username' with a value of 'root', the call with NextToken should include
...
type lookup_events_request = {
  ...
  event_category: event_category option;
   (**{|
    ...
    you do not specify [insight] as the value of [EventCategory], no Insights
    events are returned.|}*)
...
type list_insights_metric_data_request = {
  ...
  period: int option;
   (**{|
    ...
    [300], and [3600]. If you specify any other value, you will get an error.
...
(**{|
    ...
    your account. To select events for an event data store, use
...
type channel = {
  name: string option;
   (**{|
    ...
    is [aws-service-channel/service-name/custom-suffix] where [service-name]
    represents the name of the Amazon Web Services service that created the
    channel and [custom-suffix] represents the suffix created by the Amazon
...
type get_trail_status_response = {
  ...
  latest_digest_delivery_error: string option;
   (**{|
    ...
    see
    {{: https://docs.aws.amazon.com/AmazonS3/latest/API/ErrorResponses.html }Error
    ...
    policy} so that CloudTrail can write to the bucket; or create a new
  ...
  latest_notification_error: string option;
   (**{|
    ...
    the {{: https://docs.aws.amazon.com/sns/latest/dg/welcome.html }Amazon
    SNS Developer Guide}.|}*)
...
type get_insight_selectors_response = {
  ...
  insight_selectors: insight_selector list option;
   (**{|
    ...
    or event data store. [ApiErrorRateInsight] and [ApiCallRateInsight] are
...
type get_event_data_store_response = {
  partition_keys: partition_key list option;
   (**{|
    ...
    and efficiency, CloudTrail Lake organizes event data into partitions
    based on values derived from partition keys.|}*)
...
type get_channel_response = {
  ...
  name: string option;
   (**{|
    ...
    is [aws-service-channel/service-name/custom-suffix] where [service-name]
    represents the name of the Amazon Web Services service that created the
    channel and [custom-suffix] represents the suffix generated by the Amazon
    Web Services service.|}*)
...
type enable_federation_request = {
  federation_role_arn: string;
   (**{|
    ...
    exist in your account and provide the
...
type describe_trails_response = {
  trail_list: trail list option;
   (**{|
    ...
    example, [SNSTopicName] and [SNSTopicARN] are only returned in results if
    a trail is configured to send SNS notifications. Similarly, [KMSKeyId]
    only appears in results if a trail's log files are encrypted with KMS
...
type describe_trails_request = {
  include_shadow_trails: bool option;
   (**{|
    ...
    include shadow trails, organization trails in a member account and Region
...
type describe_query_response = {
  ...
  query_statistics: query_statistics_for_describe_query option;
   (**{|
    ...
    the total number of events scanned, the query run time in milliseconds,
...
type create_trail_request = {
  ...
  is_organization_trail: bool option;
   (**{|
    ...
    management account or delegated administrator account for an organization
    in Organizations.|}*)
  ...
  s3_bucket_name: string;
   (**{|
    Specifies the name of the Amazon S3 bucket designated for publishing log
    files. For information about bucket naming rules, see
...
type create_event_data_store_request = {
  ...
  advanced_event_selectors: advanced_event_selector list option;
   (**{|
    ...
    Config configuration items in your event data store, see
    {{: https://docs.aws.amazon.com/awscloudtrail/latest/userguide/lake-eds-cli.html#lake-cli-create-eds-config }Create
    an event data store for Config configuration items} in the CloudTrail
    User Guide.
    
    For more information about how to use advanced event selectors to include
    events outside of Amazon Web Services events in your event data store,
    see
    {{: https://docs.aws.amazon.com/awscloudtrail/latest/userguide/lake-integrations-cli.html#lake-cli-create-integration }Create
    an integration to log events from outside Amazon Web Services} in the
...
(**{|
    ...
    User Guide} for information about the data that is included with each
    Amazon Web Services API call listed in the log files.|}*)
